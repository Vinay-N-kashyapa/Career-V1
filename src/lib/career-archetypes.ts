...
// ── Personality questions → archetype scoring ────────────────────────
// Each option has a weights map: archetype → score contribution.
// Final archetype = highest composite score after all 6 questions.
...
export function computeArchetype(
  ...
  PERSONALITY_QUESTIONS.forEach((q) => {
    ...
    if (!option) return;
    (Object.entries(option.weights) as [ArchetypeId, number][]).forEach(([arch, w]) => {
      scores[arch] += w;
    });
  });
...
export const ROLE_SALARY: Record<string, { min: string; max: string; growth: string }> = {
