...
export function generateResumeHtml(data: ResumeFormData): string {
  const {
    ...
    summary, experiences, education, skills = { technical: '', professional: '', personal: '' }, projects, certificates, languages, declaration
  } = data;

  // Contact list
  const contactItems: string[] = [];
  if (email) contactItems.push(`<li>${email}</li>`);
  if (phone) contactItems.push(`<li>${phone}</li>`);
  if (address) contactItems.push(`<li>${address}</li>`);
  if (linkedin) {
 ...
 */
export function printResumeClientSide(data: ResumeFormData) {
