...
const PREFS_META: { key: keyof NotifPrefs; label: string; desc: string; icon: string }[] = [
...
export default function NotificationPreferences() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const d = await api.get<{ user: { notification_prefs?: Partial<NotifPrefs> } }>('/api/auth/me');
  ...
  async function save() {
    ...
    try {
      await api.patch('/api/auth/profile', { notification_prefs: prefs });
      setSaved(true);
  ...
  return (
    <div style={{
      ...
      <div style={{ display:'flex', gap:8, marginTop:14, paddingTop:14, borderTop:'1px solid var(--border)' }}>
        <button
          onClick={() => setPrefs(Object.fromEntries(PREFS_META.map(p => [p.key, true])) as unknown as NotifPrefs)}
          className="btn-ghost btn-sm"
        >
          Enable all
        </button>
        <button
          onClick={() => setPrefs(Object.fromEntries(PREFS_META.map(p => [p.key, false])) as unknown as NotifPrefs)}
          className="btn-ghost btn-sm"
        >
          Disable all
        </button>
