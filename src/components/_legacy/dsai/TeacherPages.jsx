...
import { Btn, Card, Input, Textarea, Spinner, EmptyState, Badge, Modal } from '../components/UI.jsx';
import { useToast } from '../contexts/ToastContext.jsx';
import dsaiLogo from '../assets/dsaiLogo.js';
import { exportResultsExcel } from '../utils/excelUtils.js';
...
export function TeacherLogin({ onBack, onSuccess }) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, background: 'var(--bg-primary)' }}>
      ...
      <Card style={{ maxWidth: 420, width: '100%', position: 'relative', zIndex: 1, padding: '40px 36px', boxShadow: '0 8px 40px rgba(5,150,105,0.1)' }}>
        ...
        <form onSubmit={handleLogin}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <button type="button" onClick={() => setShowPass(s => !s)}
              style={{ position: 'absolute', right: 12, top: 38, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 12, padding: '2px 6px' }}>
              {showPass ? '🙈 Hide' : '👁 Show'}
            </button>
          </div>
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--bg-primary)' }}>
      ...
      <aside style={{
        ...
        <div style={{ padding: '14px 12px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'space-between', minHeight: 62, flexShrink: 0 }}>
          {!collapsed && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
              <img src={dsaiLogo} alt="DSAI" style={{ width: 32, height: 32, borderRadius: '50%', objectFit: 'cover', flexShrink: 0, border: '1.5px solid rgba(5,150,105,0.25)' }} />
          ...
          {collapsed && <img src={dsaiLogo} alt="DSAI" style={{ width: 32, height: 32, borderRadius: '50%', objectFit: 'cover', border: '1.5px solid rgba(5,150,105,0.25)' }} />}
          <button onClick={() => setCollapsed(c => !c)}
            style={{ background: 'rgba(5,150,105,0.07)', border: '1px solid rgba(5,150,105,0.15)', cursor: 'pointer', color: 'var(--success)', fontSize: 12, padding: '5px 7px', flexShrink: 0, borderRadius: 7, lineHeight: 1, fontWeight: 700, marginLeft: collapsed ? 0 : 6 }}>
            {collapsed ? '›' : '‹'}
          </button>
        </div>
        ...
        {/* Teacher info */}
        {!collapsed && (
          <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--border)', background: '#f0fdf4', flexShrink: 0 }}>
        ...
        <nav style={{ flex: 1, padding: '8px 7px', overflowY: 'auto' }}>
          {navItems.map(item => (
            <button key={item.id} onClick={() => setActiveTab(item.id)}
              ...
              onMouseEnter={e => { handleTabHover(item.id); if (activeTab !== item.id) e.currentTarget.style.background = 'rgba(5,150,105,0.05)'; }}
    ...
    <div className="fade-in">
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <select value={filterBatch} onChange={e => setFilterBatch(e.target.value)}
          style={{ padding: '8px 12px', fontSize: 13, borderRadius: 8, border: '1.5px solid var(--border)', background: 'white', fontFamily: 'var(--font-main)', color: 'var(--text-primary)' }}>
      ...
      <div className="fade-in">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 18 }}>
          {[['Student', selected.studentName], ['Exam', selected.examTitle], ['Auto Score', selected.score || '—'], ['Tab Switches', String(selected.tabSwitches || 0)]].map(([k, v]) => (
            <div key={k} style={{ background: '#f8faff', padding: '10px 12px', borderRadius: 9, border: '1px solid var(--border)' }}>
        ...
        <Card>
          {entries.length === 0 ? <EmptyState icon="📝" text="No answers recorded" /> : entries.map(([key, ans]) => (
            <div key={key} style={{ marginBottom: 18, padding: '14px', background: '#f8faff', borderRadius: 10, border: '1px solid var(--border)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <input
                  ...
                  style={{ width: 80, padding: '6px 10px', borderRadius: 'var(--radius-sm)', border: '1.5px solid var(--border)', fontSize: 13, fontFamily: 'var(--font-main)', background: '#f8faff', color: 'var(--text-primary)' }}
                />
    ...
    <div className="fade-in">
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search student name or ID…"
          style={SEARCH_INPUT_STYLE} />
        <select value={filterBatch} onChange={e => setFilterBatch(e.target.value)}
          style={{ padding: '8px 12px', fontSize: 13, borderRadius: 9, border: '1.5px solid var(--border)', background: 'white', fontFamily: 'var(--font-main)' }}>
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 18 }}>
              {[
                ...
                <div key={k} style={{ background: '#f8faff', padding: '9px 11px', borderRadius: 8, border: '1px solid var(--border)' }}>
                  <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-muted)', marginBottom: 3 }}>{k}</div>
                  <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--text-primary)' }}>{v}</div>
                </div>
              ))}
    ...
    <div className="fade-in">
      ...
      <Card style={{ marginBottom: 18 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <Input label="Title *" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} placeholder="e.g. Unit 1 Notes" style={{ marginBottom: 0 }} />
          <Input label="Subject" value={form.subject} onChange={e => setForm(p => ({ ...p, subject: e.target.value }))} placeholder="e.g. Data Science" style={{ marginBottom: 0 }} />
          <div>
            ...
            <select value={form.batch} onChange={e => setForm(p => ({ ...p, batch: e.target.value }))}
              style={{ width: '100%', padding: '10px 14px', border: '1.5px solid var(--border)', borderRadius: 8, fontSize: 14, fontFamily: 'var(--font-main)', background: '#f8faff' }}>
          ...
          <div style={{ gridColumn: '1/-1' }}>
            <Textarea label="Description" value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Optional description…" rows={2} style={{ marginBottom: 0 }} />
          </div>
          <div style={{ gridColumn: '1/-1' }}>
            ...
            <input ref={fileRef} type="file" accept=".pdf,.ppt,.pptx,.doc,.docx,.jpg,.jpeg,.png,.gif,.webp"
              ...
              style={{ width: '100%', padding: '10px 14px', border: '1.5px solid var(--border)', borderRadius: 8, fontSize: 13, fontFamily: 'var(--font-main)', background: '#f8faff' }} />
            {file && (
              <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: '#f0fdf4', borderRadius: 8, border: '1px solid rgba(5,150,105,0.2)' }}>
        ...
        <div style={{ marginTop: 16 }}>
          <Btn variant="success" onClick={handleUpload} disabled={saving}>
            {saving ? <><Spinner size={14} color="white" /> Uploading…</> : '📤 Upload Note'}
          </Btn>
        </div>
      </Card>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search…"
          style={{ ...SEARCH_INPUT_STYLE, flex: 'none', width: 220 }} />
      </div>

      {displayed.length === 0 ? (
        <Card><EmptyState icon="📚" text="No notes uploaded yet" /></Card>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {displayed.map(n => (
            <div key={n.id} style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 12, padding: '13px 16px', display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 1px 4px rgba(37,99,235,0.05)' }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                {n.fileUrl && <Btn variant="ghost" size="sm" onClick={() => dlFile(n.fileUrl, n.fileName)}>📥 Download</Btn>}
      ...
      {notifs.length === 0 ? <Card><EmptyState icon="🔔" text="No notifications yet" /></Card> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {[...notifs].reverse().map(n => (
            <div key={n.id} style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 12, padding: '13px 16px', boxShadow: '0 1px 4px rgba(37,99,235,0.05)' }}>
  ...
  return (
    <div className="fade-in">
      ...
      {news.length === 0 ? <Card><EmptyState icon="📰" text="No news posted yet" /></Card> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[...news].reverse().map(n => (
            <div key={n.id} style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 18px', boxShadow: '0 1px 4px rgba(37,99,235,0.05)' }}>
    ...
    <div className="fade-in">
      ...
      <div style={{ display:'flex', gap:10, marginBottom:16, flexWrap:'wrap' }}>
        ...
        <select value={batch} onChange={e=>setBatch(e.target.value)}
          style={{ padding:'8px 12px', fontSize:13, borderRadius:8, border:'1.5px solid var(--border)', background:'white', fontFamily:'var(--font-main)' }}>
          <option value="all">All Batches</option>
...
function TeacherPapers() {
    ...
    return (
      <div className="fade-in">
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
          ...
          <Btn variant="ghost" size="sm" onClick={() => setActive(null)}>← Back</Btn>
        </div>
        <Card style={{ padding:0 }}>
          {qs.length===0 ? <div style={{padding:28}}><EmptyState icon="📝" text="No questions"/></div> : (
            <table className="data-table">
              ...
              <tbody>
                {qs.map((q,i)=>(
                  <tr key={i}>
                    ...
                    <td><Badge type="info">{q.type||'mcq'}</Badge></td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <div className="fade-in">
      ...
      <Card style={{ padding:0 }}>
        {papers.length===0 ? <div style={{padding:28}}><EmptyState icon="📝" text="No papers yet"/></div> : (
          <table className="data-table">
            ...
            <tbody>
              {papers.map(p=>(
                <tr key={p.id}>
                  ...
                  <td><Btn variant="ghost" size="sm" onClick={() => setActive(p)}>👁 View</Btn></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function TeacherExams() {
  ...
  const load = useCallback(() => { DB.getAll('exam_schedule').then(setExams).catch(err => { console.error('Exams load error:', err); setExams([]); }); }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const ExamList = ({ items, label, color }) => (
    <Card style={{ marginBottom:14, border:`1.5px solid ${color}22` }}>
      <h3 style={{ fontWeight:700, fontSize:14, color, marginBottom:12 }}>{label} ({items.length})</h3>
      {items.length===0 ? <EmptyState icon="🗓️" text="None"/> : items.map(e=>(
        <div key={e.id} style={{ padding:'10px 0', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          ...
          <Badge type={label.includes('Live')?'danger':label.includes('Upcoming')?'info':'success'}>{label.split(' ')[0]}</Badge>
        </div>
      ))}
    </Card>
  );
...
function TeacherMessages() {
  ...
  const toast = useToast();
  const load = useCallback(() => { DB.getAll('student_messages').then(setMessages).catch(err => { console.error('Messages load error:', err); setMessages([]); }); }, []);
  useEffect(() => { load(); }, [load]);
