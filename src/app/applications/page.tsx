...
export default function MyApplicationsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const d = await api.get<{ applications: Application[] }>('/api/opportunities/applications');
  ...
  const visible = useMemo(() => {
    ...
    if (filter === 'active') return apps.filter(a => !['rejected','withdrawn'].includes(a.status));
  ...
  return (
    <div style={{ maxWidth: 980, margin: '0 auto' }} className="animate-fade-in">
      ...
      {/* List */}
      {loading && (
        <div style={emptyStyle}>Loading your applications…</div>
      )}
      ...
      {!loading && apps.length === 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <NextStepCard
            ...
            description="See jobs and internships matched to your skills, ATS score, and target role."
            href="/opportunities"
            ctaLabel="Browse →"
          />
          <NextStepCard
            ...
            description="Recruiters scan ATS scores in 6 seconds. A higher score = more matches."
            href="/resume"
...
function ApplicationCard({ app }: { app: Application }) {
  ...
  const stipend =
    app.stipend_min && app.stipend_max
    ...
    <div style={{
      ...
      {app.required_skills && app.required_skills.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 10 }}>
          {app.required_skills.slice(0, 6).map(s => (
            <span key={s} className="skill-tag" style={{ fontSize: 10 }}>{s}</span>
          ))}
...
function FunnelStat({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div>
