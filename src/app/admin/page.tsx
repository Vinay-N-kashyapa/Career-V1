...
type AdminTab = 'dashboard' | 'users' | 'fraud' | 'stats' | 'audit' | 'broadcast';
...
function ScoreOverrideModal({ user, onClose, onDone }: { user: UserRow; onClose: () => void; onDone: () => void }) {
  const [field, setField] = useState('trust_score');
  ...
  async function submit() {
    ...
    if (isNaN(numVal) || numVal < 0 || numVal > maxVal) { setError(`Value must be 0–${maxVal}`); return; }
    if (reason.length < 10) { setError('Reason must be at least 10 characters'); return; }
    setError(''); setSubmitting(true);
    try {
      await api.post(`/api/admin/users/${user.id}/score-override`, { field, value: numVal, reason });
      onDone(); onClose();
    } catch (e: any) {
...
function BanModal({ user, onClose, onDone }: { user: UserRow; onClose: () => void; onDone: () => void }) {
  ...
  async function confirm() {
    setLoading(true);
    try {
      await api.post(`/api/admin/users/${user.id}/suspend`, { reason: reason || 'Violation of terms' });
      onDone(); onClose();
...
export default function AdminPage() {
  ...
  const [fraudAlerts, setFraudAlerts] = useState<any>({ highTabSwitches: [], suspiciousScores: [] });
  ...
  // Broadcast state
  const [bcast, setBcast] = useState({ title: '', message: '', type: 'info', targetRole: '' });
  const [toast, setToast] = useState<{ msg: string; type: 'success' | 'error' } | null>(null);

  // Modals
  const [scoreModal, setScoreModal] = useState<UserRow | null>(null);
  ...
  useEffect(() => {
    if (tab === 'users') { setPage(0); setUsers([]); fetchUsers(0); }
    if (tab === 'fraud') fetchFraudAlerts();
    if (tab === 'stats') fetchPlatformStats();
  ...
  async function fetchUsers(p = page) {
    ...
    try {
      ...
      const d = await api.get<{ users: UserRow[] }>(`/api/admin/users?${params}`);
  ...
  async function fetchFraudAlerts() {
    try {
      const d = await api.get('/api/admin/fraud-alerts');
  ...
  async function fetchPlatformStats() {
    try {
      const d = await api.get('/api/admin/platform-stats');
      setStats(d || {});
    } catch {}
  }
  ...
  async function fetchAuditLogs() {
    try {
      const d = await api.get<{ log: AuditEntry[] }>('/api/admin/audit-log');
      setAuditLog(d.log || []);
    } catch {}
  }
  ...
  async function sendBroadcast() {
    ...
    try {
      const d = await api.post<{ sent: number }>('/api/admin/broadcast', bcast);
  ...
  function handleRecalculateTrust(userId: string) {
    api.post('/api/trust/evaluate', { studentId: userId }).then(() => {
      triggerToast('Trust score recalculated successfully.');
      fetchUsers(0);
    });
  }

  function handleClearFraudAlert(studentName: string) {
    triggerToast(`Alert resolved for ${studentName}`);
    // Filter locally
    setFraudAlerts((prev: any) => ({
      highTabSwitches: prev.highTabSwitches.filter((s: any) => s.display_name !== studentName),
      suspiciousScores: prev.suspiciousScores.filter((s: any) => s.display_name !== studentName)
    }));
  }
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', paddingBottom: 60 }}>
      {scoreModal && (
        <ScoreOverrideModal
          ...
          onDone={() => { fetchUsers(0); triggerToast('Score modified and logged'); }}
        />
      )}
      {banModal && (
        <BanModal
          ...
          onDone={() => { fetchUsers(0); triggerToast('User suspended successfully.'); }}
        />
      )}
        ...
        <div>
          <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
            ...
            >
              ...
              {['student', 'recruiter', 'consultant', 'admin', 'suspended'].map(r => (
          ...
          <table className="data-table">
            ...
            <tbody>
              {loadingUsers && users.length === 0 ? (
                ...
                <tr key={u.id} style={{ opacity: u.role === 'suspended' ? 0.6 : 1 }}>
                  <td>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      {u.role === 'suspended' ? (
                        <button onClick={() => handleUnsuspend(u.id)} className="btn-primary btn-sm" style={{ fontSize: 11, background: 'var(--green)', borderColor: 'var(--green)' }}>
                          Reactivate
                        </button>
                      ) : (
                        <button onClick={() => setBanModal(u)} className="btn-ghost btn-sm" style={{ color: 'var(--coral)', border: 'none' }}>
                          🚫 Suspend
                        </button>
                      )}
                    </div>
      ...
      {tab === 'fraud' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          <div>
            ...
            {fraudAlerts.highTabSwitches.length === 0 ? (
              <div style={{ fontSize: 11, color: 'var(--t3)', fontStyle: 'italic' }}>No tab switches flagged.</div>
            ) : (
              fraudAlerts.highTabSwitches.map((s: any, i: number) => (
                <div key={i} style={{ background: 'var(--bg2)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 12, padding: 14, marginBottom: 10 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                    ...
                    <button onClick={() => api.patch(`/api/admin/users/${s.display_name}/role`, { role: 'suspended' }).then(() => triggerToast('User suspended'))} className="btn-ghost btn-sm" style={{ fontSize: 10, color: 'var(--coral)' }}>
                      🚫 Suspend
                    </button>
                  </div>
                </div>
          ...
          <div>
            ...
            {fraudAlerts.suspiciousScores.length === 0 ? (
              <div style={{ fontSize: 11, color: 'var(--t3)', fontStyle: 'italic' }}>No suspicious score jumps found.</div>
            ) : (
              fraudAlerts.suspiciousScores.map((s: any, i: number) => (
                <div key={i} style={{ background: 'var(--bg2)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 12, padding: 14, marginBottom: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ fontWeight: 600 }}>{s.display_name}</div>
          ...
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: 18 }}>
            ...
            <div style={{ display: 'flex', gap: 8, height: 160, alignItems: 'flex-end', paddingBottom: 10, borderBottom: '1px solid var(--border)' }}>
              {[
                ...
                <div key={bar.d} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 12 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              >
                ...
                {['score_override', 'suspend_user', 'delete_user', 'broadcast'].map(act => (
          ...
          {filteredAudit.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {filteredAudit.map((entry, i) => (
                <div key={entry.id || entry._id || i} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 14, display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  <div style={{
      ...
      {tab === 'broadcast' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          ...
          <div>
            ...
            {(!bcast.title && !bcast.message) ? (
              <div style={{ padding: 20, border: '1px dashed var(--border)', borderRadius: 12, textAlign: 'center', color: 'var(--t3)', fontSize: 12 }}>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  border: `1px solid ${
                    bcast.type === 'success' ? 'var(--green)' : bcast.type === 'error' ? 'var(--coral)' : bcast.type === 'warning' ? 'var(--amber)' : 'var(--blue)'
                  }`,
...
// ── Shared styles ─────────────────────────────────────────────────────────────
const overlay: React.CSSProperties = { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 };
const modalBox: React.CSSProperties = { background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: 24, width: 460, maxWidth: '100%', boxShadow: 'var(--shadow-lg)' };
const modalHeader: React.CSSProperties = { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18 };
const modalTitle: React.CSSProperties = { fontFamily: 'var(--font-display)', fontSize: 16, fontWeight: 800, color: 'var(--t1)' };
const closeBtn: React.CSSProperties = { background: 'none', border: 'none', cursor: 'pointer', color: 'var(--t3)', fontSize: 22, lineHeight: 1 };
const lbl: React.CSSProperties = { display: 'block', marginBottom: 5, fontSize: 11, fontWeight: 700, color: 'var(--t3)', textTransform: 'uppercase', letterSpacing: '0.6px' };
const errorBox: React.CSSProperties = { padding: '8px 12px', background: 'var(--coral-light)', border: '1px solid var(--coral)', borderRadius: 'var(--radius)', fontSize: 12.5, color: 'var(--coral)' };
