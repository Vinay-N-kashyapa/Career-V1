    ...
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: 20, marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input
            ...
            style={{ flex: 1, padding: '8px 12px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg2)', color: 'var(--t1)', fontSize: 13 }}
          />
          <button
            onClick={() => registerNumber && linkMutation.mutate(registerNumber)}
            disabled={!registerNumber || linkMutation.isPending}
            style={{ padding: '8px 16px', borderRadius: 8, background: 'var(--accent)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 13 }}
          >
      ...
      {isLoading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 16 }}>
          ...
          <div>
            {!selectedStudent ? (
              <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: 40, textAlign: 'center', color: 'var(--t3)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
                  {[
                    ...
                    <div key={card.label} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 12px', textAlign: 'center' }}>
                ...
                <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 16 }}>
                  ...
                  {overview.recentExams?.length === 0 ? (
                    <p style={{ color: 'var(--t3)', fontSize: 13 }}>No exams taken yet</p>
                  ) : (
                    overview.recentExams?.map((exam, i) => (
                ...
                {/* Weak areas */}
                {overview.profile?.weak_areas?.length > 0 && (
                  <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 16 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10 }}>Areas to Improve</div>
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      {overview.profile.weak_areas.map((area: string) => (
                        <span key={area} style={{ padding: '3px 10px', borderRadius: 20, background: 'var(--red-light)', color: 'var(--coral)', fontSize: 12 }}>
                          {area}
                        </span>
                      ))}
                    </div>
