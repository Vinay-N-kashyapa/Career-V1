...
export default function PersonalityPage() {
  ...
  const [mode,      setMode]      = useState<'report'|'challenge'|'analyze'>('report');
  ...
  async function fetchReport() {
    setLoading(true);
    const d = await api.get<{traits:Record<string,number>}>('/api/personality/report');
    setTraits(d.traits || {});
    setLoading(false);
  }

  async function getChallenge(type: string) {
    const d = await api.post<Record<string,unknown>>('/api/personality/session', { challengeType: type });
    setChallenge(d); setMode('challenge'); setResponse(''); setAnalysis(null);
  }
  ...
  async function analyzeResponse() {
    ...
    setAnalyzing(true);
    try {
      const d = await api.post<{analysis:unknown}>('/api/personality/analyze', { text: response, sessionType: 'speaking' });
      setAnalysis((d as any).analysis || null);
    } catch {}
    setAnalyzing(false);
    fetchReport();
  }
  ...
  return (
    <div style={{ maxWidth: 980, margin: '0 auto' }} className="animate-fade-in">
      ...
      <div style={{ display:'flex', gap:8, marginBottom:24, background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:12, padding:6, width:'fit-content' }}>
        {([['report','📊 Trait Report'],['challenge','🎙️ Speaking Challenge'],['analyze','🔍 Analyze']] as const).map(([t, label]) => (
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
          ...
          <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:16, overflow:'hidden', boxShadow:'var(--shadow-sm)' }}>
            ...
            <div style={{ padding:'16px 20px', display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(160px,1fr))', gap:10 }}>
              {CHALLENGES.map(c => (
                <button key={c.type} onClick={() => getChallenge(c.type)} style={{
      ...
      {mode === 'challenge' && challenge && (
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          ...
          <PinsEarnNotice earnAmount={0} activity="personality analysis" description="10 pins per AI analysis" />
          ...
          {analysis && (() => {
            const a = analysis as { trait_scores?:Record<string,number>; strengths?:string[]; improvements?:string[]; filler_words?:string[]; summary?:string; };
            return (
              <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:16, overflow:'hidden', boxShadow:'var(--shadow-sm)' }}>
                <div className="content-card-header">
