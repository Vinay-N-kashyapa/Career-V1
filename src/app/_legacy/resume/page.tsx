...
import { ResumeFormData, emptyFormData } from '@/components/career/ResumeForm.types';
import PinsGate from '@/components/pins/PinsGate';
import { useCareerOS } from '@/lib/context/CareerOSContext';
import { useAuth } from '@/lib/context/AuthContext';
import { printResumeClientSide } from '@/lib/printResume';
...
// ── AI improvement response shape ─────────────────────────────────────────────
interface Improvements {
  summary?:             string;
  experience_bullets?:  Array<{ role: string; company: string; bullets: string[] }>;
...
export default function ResumePage() {
  ...
  const [initialData,  setInitialData]  = useState<ResumeFormData | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await api.get<{ data?: ResumeFormData; resumeId?: string }>('/api/resume/structured/me');
        if (!cancelled) {
  ...
  async function handleSave(data: ResumeFormData) {
    ...
    try {
      const res = await api.post<{ ok: boolean; resumeId: string }>('/api/resume/structured', data);
      setInitialData(data);
      if (res.resumeId) setResumeId(res.resumeId);
      setSaveState('saved');
      setResumeGenerated(true);
      const skillTags = data.skills?.technical ? data.skills.technical.split(',').map((s: string) => s.trim()).filter(Boolean) : [];
  ...
  const [portfolioTheme, setPortfolioTheme] = useState<'modern' | 'advanced'>('modern');
  ...
  const handleGenerateAIAssets = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        generateFusedRoadmap(skillTags, weakAreas);
        toast.success('Resume & CV Generated! 💼', 'Targeted quests have been added to your roadmap pipeline.');
      }, 500);
  ...
  async function handleImprove() {
    ...
    setImprovements(null);
    try {
      const res = await api.post<{ ok: boolean; improvements: Improvements; currentScore: number }>(
        `/api/resume/structured/${resumeId}/enhance`,
        {}
      );
  ...
  async function handleDownloadPdf() {
    ...
    setDownloadError('');
    try {
      printResumeClientSide(initialData);
    } catch (e: unknown) {
  ...
  const BuildActions = () => (
    <div style={{
      ...
      <PinsGate featureKey="resume_enhance" onUnlocked={handleImprove}>
        <button
          disabled={!resumeId || improving}
          style={{
            display: 'flex', alignItems: 'center', gap: 7,
            padding: '9px 20px', borderRadius: 'var(--radius)',
        ...
        >
          {improving ? (
            <><span style={{ display: 'inline-block', animation: 'spin 0.8s linear infinite' }}>⟳</span> Improving…</>
          ) : (
            <>✨ AI Improve</>
          )}
        </button>
          ...
          {(improvements.skills_technical || improvements.skills_professional) && (
            <div>
              ...
              {improvements.skills_technical && (
                <SuggestionBlock label="Technical Skills" content={improvements.skills_technical} copyText={improvements.skills_technical} compact />
              )}
              {improvements.skills_professional && (
                <SuggestionBlock label="Professional Skills" content={improvements.skills_professional} copyText={improvements.skills_professional} compact />
              )}
            </div>
          )}
          ...
          {(improvements.keyword_additions?.length || 0) > 0 && (
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {improvements.keyword_additions!.map((k, i) => (
                  <span key={i} style={{
  ...
  return (
    <>
      {!shouldShowAssets ? (
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '40px 20px' }} className="animate-fade-in">
          <div style={{
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <input 
                  type="text" 
                  placeholder="e.g. senior_developer_resume_2025.pdf"
                  value={uploadDocName}
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto' }} className="animate-fade-in">
          ...
          {tab === 'build' && (
            <>
              ...
              {loaded ? (
                <>
                  <ResumeForm initialData={initialData ?? emptyFormData()} onSave={handleSave} />
                  <ImprovementPanel />
                </>
              ) : (
...
function SuggestionBlock({ label, content, copyText, compact = false }: {
  label: string; content: string; copyText: string; compact?: boolean;
}) {
