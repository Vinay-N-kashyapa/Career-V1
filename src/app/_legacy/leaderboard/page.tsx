...
interface LeaderEntry { userId:string; displayName:string; score:number; streak:number; role:string; }
...
export default function LeaderboardPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn:  () => api.get<{ leaders:LeaderEntry[]; userRank?:number }>(`/api/analytics/leaderboard?metric=${tab}`),
    staleTime: 2 * 60 * 1000,
  });
    ...
    <div style={{ maxWidth: 760, margin: '0 auto' }} className="animate-fade-in">
      ...
      <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 18, overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
        ...
        {isLoading ? (
          ...
          <div className="empty-state-enhanced" style={{ margin: 20 }}>
