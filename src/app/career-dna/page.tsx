...
import { useCareerDna, useCareerDnaHistory, KEYS }   from '@/lib/api/hooks';
import type { HistoryPoint }                          from '@/lib/api/hooks';
import { api }                                        from '@/lib/api/client';
import { useQueryClient }                             from '@tanstack/react-query';
import { AreaChart }                                  from '@/components/ui/charts';
import { ARCHETYPES, getArchetype, type ArchetypeId } from '@/lib/career-archetypes';
...
const EVOLUTION_METRICS: { key: keyof HistoryPoint; label: string; color: string }[] = [
...
function historyToPoints(history: HistoryPoint[], metricKey: keyof HistoryPoint) {
  return history.map((h) => ({
    label: new Date(h.date).toLocaleDateString('en-IN', { month: 'short' }),
    y:     typeof h[metricKey] === 'number' ? (h[metricKey] as number) : 0,
  }));
}

// ── Archetype derivation ──────────────────────────────────────────────────────
function deriveArchetypeFromProfile(profile: Record<string, unknown>): ArchetypeId {
  const stored = profile?.career_dna_archetype as ArchetypeId | undefined;
  if (stored && ARCHETYPES.find(a => a.id === stored)) return stored;
  const scores: Record<ArchetypeId, number> = {
    strategist: ((profile?.leadership_score  as number) || 0) * 1.2,
    builder:    ((profile?.innovation_score  as number) || 0) * 1.1 + ((profile?.consistency_score as number) || 0) * 0.3,
    visionary:  ((profile?.career_readiness  as number) || 0) + ((profile?.innovation_score as number) || 0) * 0.8,
    operator:   ((profile?.consistency_score as number) || 0) * 1.5,
    analyst:    ((profile?.adaptability_score as number) || 0) * 1.2,
    creator:    ((profile?.communication_score as number) || 0) * 1.3,
  };
  return (Object.entries(scores) as [ArchetypeId, number][]).sort((a, b) => b[1] - a[1])[0][0];
}
...
export default function CareerDNAPage() {
  ...
  const { data: dnaData, isLoading: dnaLoading } = useCareerDna();
  const { data: histData, isLoading: histLoading } = useCareerDnaHistory(6);
  const [calculating,   setCalculating]   = useState(false);
  const [editArchetype, setEditArchetype] = useState(false);
  const [activeMetric, setActiveMetric]  = useState<keyof HistoryPoint>('career_dna_score');

  const profile     = (dnaData as { profile?: Record<string, unknown> })?.profile || {} as Record<string, unknown>;
  const dnaScore    = Math.round((profile.career_dna_score as number) || 0);
  const archetypeId = deriveArchetypeFromProfile(profile);
  const archetype   = getArchetype(archetypeId);
  const streak      = (profile.mission_streak as number) || 0;
  ...
  const { earnPins: cOSEarnPins } = useCareerOS();
  async function recalculate() {
    setCalculating(true);
    try {
      await api.post('/api/career-dna/calculate', {});
      await qc.invalidateQueries({ queryKey: KEYS.careerDna });
      await qc.invalidateQueries({ queryKey: KEYS.careerDnaHistory });
    } catch { /* swallow */ }
    finally { setCalculating(false); }
  }

  async function setArchetype(id: ArchetypeId) {
    try {
      await api.patch('/api/career-dna/archetype', { archetype: id });
      await qc.invalidateQueries({ queryKey: KEYS.careerDna });
    } catch {}
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 16 }} className="animate-fade-in">
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {archetype.traits.map(t => (
                <span key={t} style={{ padding: '4px 12px', borderRadius: 100, fontSize: 11.5, fontWeight: 600, background: `${archetype.color}1a`, color: archetype.color, border: `1px solid ${archetype.color}44` }}>{t}</span>
              ))}
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'flex-start' }}>
            <button onClick={() => setEditArchetype(e => !e)} className="btn-ghost btn-sm">✏ Change Archetype</button>
            <PinsGate featureKey="career_dna_calc" onUnlocked={recalculate}>
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
              {DNA_DIMENSIONS.map((dim) => (
                <DimensionGauge key={dim.label} label={dim.label} value={Math.round((profile[dim.key] as number) || 0)} color={dim.color} icon={dim.icon} />
              ))}
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 16 }}>
        ...
        <div className="glass-card card-hover" style={{ padding: 20 }}>
          ...
          {histLoading ? (
            ...
            <AreaChart
              ...
              color={activeMetricCfg.color}
              height={180}
              yMax={100}
              yMin={0}
            />
          ) : (
            <div style={{ height: 180, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--t3)', fontSize: 13, gap: 8 }}>
        ...
        <div className="glass-card card-hover" style={{ padding: 20, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
            ...
            {streak > 0 && (
              <span style={{ fontSize: 11, color: 'var(--amber)', fontFamily: 'var(--font-mono)', fontWeight: 700 }}>🔥 {streak}-day streak</span>
            )}
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12 }}>
          {ARCHETYPES.map((a) => {
            ...
            return (
              <button
                key={a.id}
                onClick={() => editArchetype ? setArchetype(a.id) : undefined}
                style={{
...
function DimensionGauge({ label, value, color, icon }: { label: string; value: number; color: string; icon: string }) {
...
const S = {
  ...
  cardLabel: {
    fontSize: 10.5, letterSpacing: '0.8px', textTransform: 'uppercase' as const,
