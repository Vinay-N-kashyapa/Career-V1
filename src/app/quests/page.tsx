...
export default function QuestsPage() {
  ...
  const { roadmapGenerated, resumeGenerated, completedQuests, pins, onboardingStep } = useCareerOS();
    ...
    return (
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '80px 20px', textAlign: 'center' }} className="animate-fade-in">
        <div style={{
          ...
          <Link href="/career-builder" className="btn-primary" style={{ display: 'inline-flex', padding: '12px 24px', fontSize: 13.5 }}>
  ...
  const getQuestMetadata = (id: string, type: string) => {
    switch(id) {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
          ...
          const fallbackModules: Module[] = [
            {
              ...
              difficulty: 'Beginner',
              estimatedWeeks: 2,
              quests: QUESTS_REGISTRY.slice(0, 3) as any[]
            },
            {
              id: 'fallback_mod_2',
              ...
              estimatedWeeks: 3,
              quests: QUESTS_REGISTRY.slice(3, 5) as any[]
            },
            {
              id: 'fallback_mod_3',
              ...
              difficulty: 'Advanced',
              estimatedWeeks: 3,
              quests: QUESTS_REGISTRY.slice(5) as any[]
            }
          ];
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
                  {(mod.quests || []).map((ex, exIdx) => {
                    ...
                    return (
                      <div
                        key={ex.id}
                        onClick={() => !locked && router.push(`/quests/${ex.id}`)}
                        className={`glass-card ${locked ? '' : 'card-hover'}`}
                        style={{
                          padding: '20px',
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--border)', paddingTop: 12, marginTop: 12 }}>
                          ...
                          {completed ? (
                            <span style={{ fontSize: 9.5, color: 'var(--green)', fontWeight: 800, fontFamily: 'var(--font-mono)' }}>DONE</span>
                          ) : locked ? (
                            <span style={{ fontSize: 9.5, color: 'var(--t4)', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>LOCKED</span>
                          ) : (
                            <span style={{ fontSize: 9.5, color: 'var(--accent)', fontWeight: 800, fontFamily: 'var(--font-mono)', display: 'flex', alignItems: 'center', gap: 2 }}>
                              START ➔
                            </span>
                          )}
                        </div>
        ...
        {completedCount > 0 && (
          <div className="glass-card" style={{
            ...
            <Link href="/interview" className="btn-primary" style={{ display: 'inline-flex', padding: '10px 20px', fontSize: 13 }}>
              Go to AI Interviews ➔
            </Link>
          </div>
        )}
