...
exports.default = CareerTwinPage;
...
const getOnboardingQuestions = (teacherName) => [
...
const QUESTS = [
    {
        ...
        stages: [
            { title: 'Resume Optimisation', desc: 'Get your resume ATS score above 60.', href: '/career-assets', icon: '📄', isComplete: cOS => cOS.vaultItems.some((itm) => itm.used_in_resume) },
            ...
            { title: 'Portfolio Upload', desc: 'Deploy living portfolio website.', href: '/career-assets', icon: '🚀', isComplete: cOS => cOS.vaultItems.some((itm) => itm.used_in_portfolio) },
    ...
    },
    {
        ...
        stages: [
            { title: 'Resume Optimisation', desc: 'Generate career assets with AI.', href: '/career-assets', icon: '📄', isComplete: cOS => cOS.vaultItems.some((itm) => itm.used_in_resume) },
            ...
            { title: 'SQL Mastery', desc: 'Upload evidence of database projects.', href: '/vault', icon: '🗄', isComplete: cOS => cOS.vaultItems.some((itm) => itm.skill_tags.some((s) => s.toLowerCase().includes('sql') || s.toLowerCase().includes('database'))) },
...
function CareerTwinPage() {
    ...
    const { earnPins: twinEarnPins, spendPins: twinSpendPins, canAfford: twinCanAfford } = (0, CareerOSContext_1.useCareerOS)();
    ...
    const [onboardingComplete, setOnboardingComplete] = (0, react_1.useState)(onboardingAnswers.hasCompleted);
    const [step, setStep] = (0, react_1.useState)(0);
    const [answers, setAnswers] = (0, react_1.useState)(onboardingAnswers);
    ...
    const chatBottomRef = (0, react_1.useRef)(null);
    (0, react_1.useEffect)(() => {
    ...
    return (<div style={{ maxWidth: 1040, margin: '0 auto' }} className="animate-fade-in">
      ...
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
        ...
        {onboardingComplete && (<PinsGate_1.default featureKey="career_twin" onUnlocked={() => {
      ...
      {!onboardingComplete && (<div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20, alignItems: 'start' }}>
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 20, display: 'flex', flexDirection: 'column', height: 480, overflow: 'hidden', boxShadow: 'var(--shadow-md)' }}>
            ...
            <div style={{ flex: 1, padding: 18, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 12, background: 'var(--bg)' }}>
              {chatHistory.map((msg, i) => (<div key={i} style={{ display: 'flex', justifyContent: msg.sender === 'ai' ? 'flex-start' : 'flex-end' }}>
                  <div style={{
                    maxWidth: '80%',
                    padding: '10px 14px',
                    borderRadius: 12,
              ...
              {simulating && (<div style={{ display: 'flex', justifyContent: 'flex-start' }}>
            ...
            <div style={{ padding: '12px 16px', background: 'var(--bg2)', borderTop: '1px solid var(--border)', display: 'flex', gap: 10 }}>
              <input type="text" value={inputVal} onChange={e => setInputVal(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSendAnswer()} placeholder={ONBOARDING_QUESTIONS[step]?.placeholder || "Type your response..."} style={{ flex: 1, padding: '8px 14px', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 10, color: 'var(--t1)', fontSize: 12.5, outline: 'none' }}/>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 20, padding: 20 }}>
              ...
              <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {ONBOARDING_QUESTIONS.map((q, idx) => (<div key={q.id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: idx <= step ? 'var(--t1)' : 'var(--t3)' }}>
                    <div style={{
      ...
      {onboardingComplete && (<div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          
          {simulating && (<div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 20, padding: 48, textAlign: 'center' }}>
          ...
          {!simulating && (<>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 20 }}>
                ...
                ].map(card => (<div key={card.label} style={{ background: 'var(--bg2)', padding: '12px 14px', borderRadius: 12, border: '1px solid var(--border)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                  <div style={{ background: 'var(--bg3)', borderRadius: 14, padding: 16 }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      {(onboardingAnswers.skills ? onboardingAnswers.skills.split(',') : ['React', 'JavaScript', 'HTML5', 'CSS3', 'Git']).map(skill => (<span key={skill} style={{ fontSize: 11, padding: '4px 10px', borderRadius: 20, background: 'var(--green-light)', color: 'var(--green)', border: '1px solid rgba(5,150,105,0.15)', fontWeight: 600 }}>{skill.trim()}</span>))}
                    </div>
                  </div>
                  <div style={{ background: 'var(--bg3)', borderRadius: 14, padding: 16 }}>
                    <div style={{ color: 'var(--coral)', fontWeight: 700, fontSize: 12, marginBottom: 8 }}>⚠️ Urgent Missing Skills (Close these first)</div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      {baseMissingSkills.map(skill => (<span key={skill} style={{ fontSize: 11, padding: '4px 10px', borderRadius: 20, background: 'var(--coral-light)', color: 'var(--coral)', border: '1px solid rgba(220,38,38,0.15)', fontWeight: 600 }}>{skill}</span>))}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: 10, margin: '10px 0' }}>
                ...
                ].map((p, idx) => (<button key={idx} onClick={() => setSelected(idx)} style={{
                  ...
                  {QUESTS.map(quest => {
                    ...
                    return (<div key={quest.id} style={{
                        ...
                        <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 6, flex: 1 }}>
                          {stageResults.map((s) => (<div key={s.index} style={{
                              ...
                              {s.completed ? (<span style={{ fontSize: 9.5, fontFamily: 'var(--font-mono)', fontWeight: 700, color: 'var(--green)' }}>Complete</span>) : s.index === currentStageIdx + 1 ? (<link_1.default href={s.href} style={{ fontSize: 10, fontFamily: 'var(--font-mono)', fontWeight: 700, color: quest.color, textDecoration: 'none', padding: '3px 8px', background: `${quest.color}18`, borderRadius: 4 }}>
                                  Start ➔
                                </link_1.default>) : (<span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: 'var(--t4)' }}>🔒 Locked</span>)}
                            </div>))}
                        </div>
              ...
              <div style={{ background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 20, padding: 20 }}>
                ...
                ].map((act, i) => (<div key={i} style={{ display: 'flex', gap: 8, fontSize: 12.5, marginBottom: 5 }}>
