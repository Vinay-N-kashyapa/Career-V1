...
function MiniLineChart({ data, color, height = 40 }) {
    if (!data || data.length < 2)
        return <div style={{ height, background: 'var(--bg3)', borderRadius: 6 }}/>;
...
function StatCard({ icon, label, value, sub, color, trend, href }) {
    const card = (<div className="score-card card-hover" style={{ cursor: href ? 'pointer' : 'default' }}>
      <div className="sc-header">
        <div className="sc-icon-wrap" style={{ background: `${color}18` }}>
          <span style={{ fontSize: 16 }}>{icon}</span>
        </div>
        {trend !== undefined && (<span style={{ fontSize: 11, fontWeight: 600, padding: '2px 7px', borderRadius: 6, background: trend >= 0 ? 'var(--green-light)' : 'var(--coral-light)', color: trend >= 0 ? 'var(--green)' : 'var(--coral)' }}>
    ...
    return href ? <link_1.default href={href} style={{ textDecoration: 'none' }}>{card}</link_1.default> : card;
}
function AnalyticsPage() {
    ...
    return (<div style={{ maxWidth: 1080, margin: '0 auto' }} className="animate-fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 20 }}>
        ...
        ].map(chart => (<div key={chart.label} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: 16, boxShadow: 'var(--shadow-sm)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 20 }}>
        ...
        ].map(item => (<div key={item.label} style={{ marginBottom: 11 }}>
        ...
        ].map(item => (<div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: '1px solid var(--border)' }}>
              <span style={{ fontSize: 12, color: 'var(--t2)' }}>{item.label}</span>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        ].map(l => (<link_1.default key={l.href} href={l.href} style={{ textDecoration: 'none' }}>
