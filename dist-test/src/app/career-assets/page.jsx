...
exports.default = CareerAssetsStudioPage;
...
function CareerAssetsStudioPage() {
    const { vaultItems, updateVaultItem, addXp, spendPins, canAfford } = (0, CareerOSContext_1.useCareerOS)();
    ...
    const [activeAsset, setActiveAsset] = (0, react_1.useState)('ats');
    const [template, setTemplate] = (0, react_1.useState)('ats-pure');
    ...
    return (<div style={{ maxWidth: 1200, margin: '0 auto' }} className="animate-fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '360px minmax(0, 1fr)', gap: 24, alignItems: 'start' }}>
        ...
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 20, padding: 20, boxShadow: 'var(--shadow-sm)' }}>
          ...
          {/* Empty Vault CTA */}
          {vaultItems.length === 0 ? (<div style={{ textAlign: 'center', padding: '24px 16px', background: 'var(--card)', borderRadius: 14, border: '1px dashed var(--border)', marginBottom: 20 }}>
              ...
              <link_1.default href="/vault" className="btn-primary btn-sm" style={{ display: 'inline-flex', width: '100%', justifyContent: 'center' }}>
                + Add Documents in Vault
              </link_1.default>
            </div>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
              {vaultItems.map(item => (<label key={item.id} style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: 12,
                    padding: '12px 14px',
                  ...
                  <input type="checkbox" checked={selectedIds.includes(item.id)} onChange={() => handleToggle(item.id)} style={{ accentColor: 'var(--accent)', cursor: 'pointer' }}/>
          ...
          {vaultItems.length > 0 && (<div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <select value={template} onChange={(e) => setTemplate(e.target.value)} className="form-input" style={{ fontSize: 12.5, height: 36, padding: '4px 8px' }}>
              ...
              <PinsGate_1.default featureKey="career_assets" onUnlocked={triggerGenerate}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Asset selectors */}
          {generated && (<div style={{ display: 'flex', gap: 6, background: 'var(--bg3)', padding: 4, borderRadius: 12, border: '1px solid var(--border)', width: 'fit-content', flexWrap: 'wrap' }}>
              {[
            ...
            ].map(tab => (<button key={tab.id} onClick={() => setActiveAsset(tab.id)} style={{
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 20, minHeight: 480, display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: 'var(--shadow-md)' }}>
            ...
            {!generated && !generating && (<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, padding: 48, textAlign: 'center' }}>
            ...
            {/* If generating */}
            {generating && (<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, padding: 48, textAlign: 'center' }}>
            ...
            switch (activeAsset) {
                case 'ats':
                    ...
                    return (<div style={{ padding: 24, fontSize: 13, color: 'var(--t1)', display: 'flex', flexDirection: 'column', gap: 16 }}>
                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                          {vaultItems.filter(itm => selectedIds.includes(itm.id)).map(itm => (<div key={itm.id} style={{ padding: '10px 12px', background: 'var(--bg3)', borderRadius: 10, border: '1px solid var(--border)' }}>
                              ...
                              <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
                                {itm.skill_tags.map(s => (<span key={s} style={{ fontSize: 9, padding: '1px 5px', borderRadius: 4, background: 'var(--bg2)', color: 'var(--t3)', fontFamily: 'var(--font-mono)' }}>{s}</span>))}
                              </div>
                            </div>))}
                        </div>
                      </div>
                ...
                case 'onepage':
                    return (<div style={{ padding: 24, fontSize: 13, color: 'var(--t1)', display: 'flex', flexDirection: 'column', gap: 14 }}>
                      ...
                      <div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 14 }}>
                          ...
                          <div>
                            ...
                            {vaultItems.filter(itm => selectedIds.includes(itm.id)).slice(0, 3).map(itm => (<div key={itm.id} style={{ fontSize: 11.5, marginBottom: 6 }}>
                ...
                case 'linkedin':
                    return (<div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
                      ...
                      <button onClick={() => {
                            ...
                            useAppStore_1.toast.success('📋 Copied to Clipboard', 'LinkedIn bio ready to paste.');
                ...
                case 'cover':
                    return (<div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
                      ...
                      <button onClick={() => {
                            ...
                            useAppStore_1.toast.success('📋 Copied to Clipboard', 'Cover letter copied.');
                        }} className="btn-primary btn-sm" style={{ width: 'fit-content' }}>
                        📋 Copy Cover Letter
                      </button>
                    </div>);
                      ...
                      <div style={{ border: '2.5px solid var(--border2)', borderRadius: 14, overflow: 'hidden', boxShadow: 'var(--shadow-sm)', background: 'var(--bg3)' }}>
                        ...
                        <div style={{ padding: 20, textAlign: 'center', background: 'var(--bg2)' }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, textAlign: 'left', maxWidth: 280, margin: '0 auto' }}>
                            ...
                            {vaultItems.filter(itm => selectedIds.includes(itm.id)).map(itm => (<div key={itm.id} style={{ fontSize: 10, padding: '6px 8px', background: 'var(--bg3)', borderRadius: 6, border: '1px solid var(--border)' }}>
                                🏆 {itm.title}
                              </div>))}
                          </div>
                        </div>
