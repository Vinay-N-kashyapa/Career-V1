...
function RecruiterPage() {
    ...
    const [activeTab, setActiveTab] = (0, react_1.useState)('candidates');
    const [candidates, setCandidates] = (0, react_1.useState)([]);
    const [analytics, setAnalytics] = (0, react_1.useState)({});
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [filters, setFilters] = (0, react_1.useState)({ minTrust: '', minAts: '', domain: '' });
    const [selectedCandidate, setSelectedCandidate] = (0, react_1.useState)(null);
    // Jobs state
    const [jobs, setJobs] = (0, react_1.useState)([]);
    const [showJobModal, setShowJobModal] = (0, react_1.useState)(false);
    const [jobForm, setJobForm] = (0, react_1.useState)(emptyJob);
    const [jobSaving, setJobSaving] = (0, react_1.useState)(false);
    // Applications state
    const [applications, setApplications] = (0, react_1.useState)([]);
    const [appReviewing, setAppReviewing] = (0, react_1.useState)(null);
    const [updatingAppStatus, setUpdatingAppStatus] = (0, react_1.useState)(null);
    // Company state
    const [companyProfile, setCompanyProfile] = (0, react_1.useState)(emptyCompany);
    const [companyLoading, setCompanyLoading] = (0, react_1.useState)(true);
    const [companySaving, setCompanySaving] = (0, react_1.useState)(false);
    const [companyEditing, setCompanyEditing] = (0, react_1.useState)(false);
    // Notifications
    ...
    async function fetchCandidates() {
        ...
        try {
            ...
            if (filters.minTrust)
                params.set('minTrust', filters.minTrust);
            if (filters.minAts)
                params.set('minAts', filters.minAts);
            if (filters.domain)
                params.set('domain', filters.domain);
            const d = await client_1.api.get(`/api/recruiter/candidates?${params}`);
    ...
    async function fetchAnalytics() {
        try {
            const d = await client_1.api.get('/api/recruiter/analytics');
    ...
    async function fetchJobs() {
        try {
            const d = await client_1.api.get('/api/recruiter/jobs');
            setJobs(d.jobs || []);
        }
        catch { }
    }
    async function fetchApplications() {
        try {
            const d = await client_1.api.get('/api/recruiter/applications');
    ...
    async function fetchCompany() {
        ...
        try {
            const d = await client_1.api.get('/api/recruiter/company');
            if (d.company) {
    ...
    async function saveCompany(e) {
        ...
        try {
            await client_1.api.post('/api/recruiter/company', companyProfile);
    ...
    async function postJob(e) {
        ...
        setJobSaving(true);
        try {
            await client_1.api.post('/api/recruiter/jobs', {
                ...jobForm,
    ...
    async function handleDeleteJob(id) {
        if (!confirm('Are you sure you want to delete this job posting?'))
            return;
        try {
            await client_1.api.delete(`/api/recruiter/jobs/${id}`);
    ...
    async function handleUpdateAppStatus(applicationId, status) {
        setUpdatingAppStatus(status);
        try {
            await client_1.api.post('/api/recruiter/applications', { applicationId, status });
    ...
    async function viewCandidate(id) {
        try {
            const d = await client_1.api.get(`/api/recruiter/candidate/${id}`);
    ...
    async function shortlist(id) {
        try {
            await client_1.api.post('/api/recruiter/shortlist', { candidateId: id });
            triggerToast('Candidate shortlisted for review');
        }
        catch { }
    }
    async function sendContactRequest(id) {
        try {
            await client_1.api.post('/api/recruiter/contact-request', { candidateId: id });
    ...
    async function scheduleInterview(candidateId) {
        ...
        try {
            await client_1.api.post('/api/recruiter/schedule-interview', {
                candidateId,
    ...
    return (<div style={{ maxWidth: 1200, margin: '0 auto', paddingBottom: 60 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border)', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', gap: 4, background: 'var(--bg3)', padding: 4, borderRadius: 'var(--radius)', border: '1px solid var(--border)', marginBottom: -1 }}>
          {['candidates', 'jobs', 'applications', 'company'].map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} style={{
        ...
        {activeTab === 'jobs' && (<button onClick={() => setShowJobModal(true)} className="btn-primary" style={{ padding: '8px 16px', fontSize: 12 }}>
            + Post New Job
          </button>)}
      </div>
      ...
      {activeTab === 'candidates' && (<>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: selectedCandidate ? '1fr 380px' : '1fr', gap: 20 }}>
            {/* Candidate List */}
            <div>
              {loading ? (<div style={{ textAlign: 'center', padding: 48, color: 'var(--t3)' }}>Searching repository...</div>) : candidates.length === 0 ? (<div className="empty-state">
                  ...
                  {candidates.map((c, i) => (<div key={c.id} onClick={() => viewCandidate(c.id)} className="glass-card card-hover" style={{
                      ...
                      <div style={{ flex: 1, minWidth: 200 }}>
                        ...
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                          {(c.skill_tags || []).slice(0, 4).map(s => (<span key={s} style={{
            ...
            {selectedCandidate && (<div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: 20, height: 'fit-content', position: 'sticky', top: 20 }}>
                ...
                ].map(s => (<div key={s.l} style={{ background: 'var(--bg3)', borderRadius: 8, padding: '8px 12px' }}>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  {(!selectedCandidate.vaultItems || selectedCandidate.vaultItems.length === 0) ? (<div style={{ fontSize: 11, color: 'var(--t3)', fontStyle: 'italic' }}>No document proofs attached yet.</div>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {selectedCandidate.vaultItems.map((item) => (<div key={item.id} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 10px', fontSize: 11 }}>
                          ...
                          {item.fileUrl && (<a href={item.fileUrl} target="_blank" rel="noreferrer" style={{ display: 'inline-block', marginTop: 4, color: 'var(--accent)', textDecoration: 'underline' }}>
                              View Document
                            </a>)}
                        </div>))}
                    </div>)}
      ...
      {activeTab === 'jobs' && (<div>
          {jobs.length === 0 ? (<div className="empty-state">
              ...
              {jobs.map(job => (<div key={job.id} className="glass-card card-hover" style={{
                  ...
                  <div>
                    ...
                    {job.skills_required && (<div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 10 }}>
                        {job.skills_required.split(',').slice(0, 4).map(s => (<span key={s} style={{ fontSize: 9.5, padding: '2px 7px', background: 'var(--bg3)', borderRadius: 4, border: '1px solid var(--border)', color: 'var(--t2)', fontFamily: 'var(--font-mono)' }}>
                            {s.trim()}
                          </span>))}
                      </div>)}
                  </div>
                  <div style={{ marginTop: 20, borderTop: '1px solid var(--border)', paddingTop: 14, display: 'flex', gap: 8, justifyContent: 'space-between' }}>
                    ...
                    }} className="btn-ghost btn-sm" style={{ fontSize: 11, padding: '4px 10px', flex: 1, justifyContent: 'center' }}>
                      ✏ Edit
                    </button>
                    <button onClick={() => {
                    ...
                    <button onClick={() => handleDeleteJob(job.id)} className="btn-ghost btn-sm" style={{ color: 'var(--coral)', border: '1px solid rgba(239, 68, 68, 0.2)', fontSize: 11, padding: '4px 10px', flex: 1, justifyContent: 'center' }}>
                      🗑 Delete
                    </button>
                  </div>
                </div>))}
      ...
      {/* ── TAB: APPLICATIONS ───────────────────────────────────────────── */}
      {activeTab === 'applications' && (<div style={{ display: 'grid', gridTemplateColumns: appReviewing ? '1fr 380px' : '1fr', gap: 20 }}>
          <div>
            {applications.length === 0 ? (<div className="empty-state">
                <div className="empty-icon">📨</div>
                ...
                <tbody>
                  {applications.map(app => (<tr key={app.id}>
          ...
          {appReviewing && (<div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: 20, height: 'fit-content', position: 'sticky', top: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                ].map(opt => (<button key={opt.id} onClick={() => handleUpdateAppStatus(appReviewing.id, opt.id)} disabled={updatingAppStatus !== null} style={{
      ...
      {/* ── TAB: COMPANY PROFILE ─────────────────────────────────────────── */}
      {activeTab === 'company' && (<div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: 24, maxWidth: 700 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 700, margin: 0 }}>Company Profile</h3>
            {!companyEditing && (<button onClick={() => setCompanyEditing(true)} className="btn-ghost btn-sm" style={{ border: '1px solid var(--border)' }}>
                ✏ Edit Profile
              </button>)}
          </div>

          {companyLoading ? (<div>Loading profile details...</div>) : !companyEditing ? (<div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {companyProfile.website && (<div style={{ fontSize: 12 }}>
              ...
              {companyProfile.about && (<div style={{ background: 'var(--bg3)', borderRadius: 10, padding: 14, fontSize: 12, lineHeight: 1.6 }}>
                  <div style={{ fontWeight: 700, color: 'var(--t2)', marginBottom: 6 }}>About us</div>
                  {companyProfile.about}
                </div>)}
            </div>) : (<form onSubmit={saveCompany} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {/* ── JOB FORM MODAL ── */}
      {showJobModal && (<div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
