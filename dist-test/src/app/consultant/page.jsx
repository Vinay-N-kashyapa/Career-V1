...
exports.default = ConsultantPage;
...
function ConsultantPage() {
    ...
    const [pipeline, setPipeline] = (0, react_1.useState)({});
    ...
    const [newTaskDueDate, setNewTaskDueDate] = (0, react_1.useState)('');
    // Add student form
    ...
    const [sessionForm, setSessionForm] = (0, react_1.useState)({
        title: '', studentId: '', studentName: '', date: '', time: '', link: '', notes: ''
    });
    const [scheduling, setScheduling] = (0, react_1.useState)(false);
    // Toast
    ...
    async function fetchPipeline() {
        setLoading(true);
        try {
            const d = await client_1.api.get('/api/consultant/pipeline');
    ...
    async function fetchAnalytics() {
        try {
            const ca = await client_1.api.get('/api/consultant/analytics');
            setAnalytics(ca || {});
        }
        catch { }
    }
    async function fetchSessions() {
        try {
            const d = await client_1.api.get('/api/consultant/sessions');
    ...
    async function addStudent(e) {
        ...
        try {
            ...
            await client_1.api.post('/api/consultant/student/add', body);
    ...
    async function updateStatus(id, status) {
        try {
            await client_1.api.patch(`/api/consultant/student/${id}`, { status });
    ...
    async function handleVerifyDocument(itemId, status) {
        if (!selectedStudent)
            return;
        try {
            await client_1.api.post(`/api/consultant/student/${selectedStudent.id}/verify-document`, { itemId, status });
    ...
    async function addTask() {
        if (!newTask.trim() || !selectedStudent)
            return;
        try {
            await client_1.api.post(`/api/consultant/student/${selectedStudent.id}/task`, {
                title: newTask,
    ...
    async function scheduleSessions(e) {
        e.preventDefault();
        ...
        try {
            ...
            await client_1.api.post('/api/consultant/sessions', {
                ...sessionForm,
                studentName
            });
    ...
    return (<div style={{ maxWidth: 1200, margin: '0 auto', paddingBottom: 60 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 20 }}>
        <div style={{ display: 'flex', gap: 4, background: 'var(--bg3)', padding: 4, borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
          {['pipeline', 'sessions', 'add', 'analytics'].map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} style={{
      ...
      {/* ── TAB: PIPELINE (KANBAN) ───────────────────────────────────────── */}
      {activeTab === 'pipeline' && (<div style={{ display: 'grid', gridTemplateColumns: selectedStudent ? '1fr 380px' : '1fr', gap: 16 }}>
          {loading ? (<div style={{ textAlign: 'center', padding: 48, color: 'var(--t3)' }}>Refreshing student pipeline...</div>) : (<div style={{ overflowX: 'auto', paddingBottom: 8 }}>
                ...
                {STAGES.map(stage => {
                    ...
                    return (<div key={stage} style={{
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, flex: 1, overflowY: 'auto' }}>
                        {students.map(s => (<div key={s.id} onClick={() => setSelectedStudent(s)} className="glass-card card-hover" style={{
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              ...
                              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }} onClick={e => e.stopPropagation()}>
                                <button title="Schedule 1:1 Consultation Session" onClick={() => {
                                setSessionForm(prev => ({ ...prev, studentId: s.id }));
                                setActiveTab('sessions');
                            }} className="btn-ghost" style={{ padding: '2px 6px', fontSize: 11, borderRadius: 6, background: 'rgba(255,255,255,0.03)' }}>
                                  📅
                                </button>
                                {s.vaultItems && s.vaultItems.length > 0 && (<span style={{ fontSize: 10, color: 'var(--green)', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 2 }}>
                                    📎{s.vaultItems.length}
                                  </span>)}
                              </div>
                            </div>
          ...
          {selectedStudent && (<div className="glass-card" style={{
                    border: '1px solid var(--border)',
                    borderRadius: 18,
                    padding: 22,
                    height: 'fit-content',
              ...
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14 }}>
                ...
                {(!selectedStudent.vaultItems || selectedStudent.vaultItems.length === 0) ? (<div style={{ fontSize: 11.5, color: 'var(--t3)', fontStyle: 'italic', padding: 8, background: 'rgba(255,255,255,0.01)', borderRadius: 8, textAlign: 'center' }}>
                    No uploads inside student vault yet.
                  </div>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {selectedStudent.vaultItems.map((item) => (<div key={item.id} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 12, padding: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14 }}>
                ...
                {(!selectedStudent.tasks || selectedStudent.tasks.length === 0) ? (<div style={{ fontSize: 11.5, color: 'var(--t3)', fontStyle: 'italic', marginBottom: 8 }}>No checklist items assigned.</div>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 10 }}>
                    {(selectedStudent.tasks || []).map((t, i) => (<div key={i} style={{ display: 'flex', gap: 8, fontSize: 11.5, alignItems: 'center', background: 'rgba(255,255,255,0.02)', padding: '6px 10px', borderRadius: 8, border: '1px solid var(--border)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 10 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <select className="form-input" style={{ flex: 1, fontSize: 11, padding: 4 }} value={newTaskPriority} onChange={e => setNewTaskPriority(e.target.value)}>
      ...
      {/* ── TAB: 1:1 SESSIONS ────────────────────────────────────────────── */}
      {activeTab === 'sessions' && (<div style={{ display: 'grid', gridTemplateColumns: '1fr 400px', gap: 20 }}>
          {/* Scheduled Sessions list */}
          <div>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 14, fontWeight: 700, marginBottom: 12 }}>Scheduled Consultations</h3>
            {sessions.length === 0 ? (<div style={{ fontSize: 12, color: 'var(--t3)', fontStyle: 'italic', padding: 20, background: 'var(--card)', borderRadius: 10 }}>
                No sessions scheduled yet. Fill out the scheduler form on the right.
              </div>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {sessions.map(s => (<div key={s.id} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 16 }}>
                    ...
                    {s.link && (<div style={{ marginTop: 8, fontSize: 11 }}>
                        Meeting URL: <a href={s.link} target="_blank" rel="noreferrer" style={{ color: 'var(--accent)' }}>{s.link}</a>
                      </div>)}
                    {s.notes && (<div style={{ marginTop: 6, fontSize: 11, color: 'var(--t3)', fontStyle: 'italic' }}>
                        Notes: {s.notes}
                      </div>)}
                  </div>))}
              </div>)}
            ...
            <form onSubmit={scheduleSessions} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div>
                ...
                <select className="form-input" style={{ width: '100%' }} value={sessionForm.studentId} onChange={e => setSessionForm(p => ({ ...p, studentId: e.target.value }))} required>
                  ...
                  {allStudents.map(s => (<option key={s.id} value={s.id}>{s.displayName} ({s.targetCountry})</option>))}
                </select>
              </div>
      ...
      {/* ── TAB: ADD STUDENT ────────────────────────────────────────────── */}
      {activeTab === 'add' && (<div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: 24, maxWidth: 700 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {[
                ...
                { k: 'budget', p: 'Financial Budget Limit', r: false },
                  ...
                  <input className="form-input" style={{ width: '100%' }} value={studentForm[f.k]} onChange={e => setStudentForm(x => ({ ...x, [f.k]: e.target.value }))} required={f.r}/>
                </div>))}
            </div>
            <div>
      ...
      {/* ── TAB: ANALYTICS ──────────────────────────────────────────────── */}
      {activeTab === 'analytics' && (<div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          {[
            ...
            ].map(s => (<div key={s.label} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 20, borderTop: `3px solid ${s.color}` }}>
