...
function QRTab() {
    ...
    const [localIP, setLocalIP] = (0, react_1.useState)('');
    const [showIPInput, setShowIPInput] = (0, react_1.useState)(false);
    ...
    function buildPhoneURL(qrToken) {
        if (localIP.trim()) {
    ...
    const createQR = (0, react_1.useCallback)(async () => {
        if (unsubRef.current) {
            unsubRef.current();
                ...
                if (row.status === 'scanned') {
                    ...
                    if (row.email && row.password) {
                        ...
                        try {
                            ...
                            await supabaseClient_1.supabase.from('qr_login_sessions').delete().eq('id', qrToken);
    ...
    return (<div style={{ textAlign: 'center' }}>
      ...
      {isLocalhost && (<div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 8, padding: '10px 12px', marginBottom: 12, textAlign: 'left' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--amber)', marginBottom: 4 }}>📡 Phone scan needs your laptop IP</div>
          {!showIPInput ? (<div style={{ fontSize: 11, color: 'var(--t2)', lineHeight: 1.5 }}>
            ...
            </div>) : (<div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 6 }}>
              ...
              <button onClick={() => { setShowIPInput(false); if (token) {
                const u = buildPhoneURL(token);
                setQrUrl(`https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(u)}&bgcolor=ffffff&color=4f46e5&margin=10&format=svg`);
            }
            else
                createQR(); }} className="btn-ghost btn-sm" style={{ fontSize: 11 }}>Apply</button>
            </div>)}
        </div>)}
      {/* QR box */}
      <div style={{
        ...
        {status === 'scanned' && (<div style={{ ...S.qrOverlay, background: 'rgba(79,70,229,0.88)' }}>
            <div style={{ fontSize: 32 }}>📱</div>
            <div style={{ fontSize: 13, fontWeight: 700 }}>Phone scanned!</div>
            <div style={{ fontSize: 11, opacity: 0.85 }}>Confirm on your phone</div>
          </div>)}
        {status === 'confirmed' && (<div style={{ ...S.qrOverlay, background: 'rgba(5,150,105,0.92)' }}>
            <div style={{ fontSize: 36 }}>✓</div>
            <div style={{ fontSize: 14, fontWeight: 700 }}>Verified!</div>
          </div>)}
        {status === 'expired' && (<div style={{ ...S.qrCenter, cursor: 'pointer' }} onClick={createQR}>
      ...
      {/* Progress bar */}
      {(status === 'ready' || status === 'scanned') && (<div style={{ height: 3, background: 'var(--bg3)', borderRadius: 2, marginBottom: 16, overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${(timeLeft / 300) * 100}%`, background: 'var(--accent)', transition: 'width 1s linear', borderRadius: 2 }}/>
        </div>)}

      {status === 'expired' && (<button onClick={createQR} className="btn-primary" style={{ width: '100%', marginBottom: 12 }}>Generate New QR</button>)}
      ...
      <div style={{ background: 'var(--bg3)', borderRadius: 10, padding: '12px 14px', textAlign: 'left', marginTop: 8 }}>
        ...
        ].map((s, i) => (<div key={i} style={S.step}>
    ...
    return (<div className="auth-page">
      <div className="auth-card animate-fade-in">
        ...
        <div style={{ textAlign: 'center', fontSize: 12.5, color: 'var(--t3)', paddingTop: 12, borderTop: '1px solid var(--border)' }}>
          <link_1.default href="/login" style={{ color: 'var(--accent)', textDecoration: 'none', fontWeight: 600 }}>Password login</link_1.default>
          {' · '}
          <link_1.default href="/signup" style={{ color: 'var(--t2)', textDecoration: 'none' }}>Create account</link_1.default>
        </div>
      </div>
    </div>);
}
// ── Styles ────────────────────────────────────────────────────────────
const S = {
    logo: { width: 38, height: 38, background: 'linear-gradient(135deg,var(--accent),var(--purple))', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 800, color: 'white', boxShadow: '0 4px 14px rgba(79,70,229,.3)' },
    qrCenter: { width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, background: 'var(--bg3)' },
    qrOverlay: { position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, color: 'white' },
    qrLabel: { fontSize: 11, color: 'var(--t3)' },
    stepsTitle: { fontSize: 10.5, fontWeight: 600, color: 'var(--t3)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 8, fontFamily: 'var(--font-mono)' },
