...
exports.default = TrustPage;
const client_1 = require("@/lib/api/client");
const react_1 = require("react");
const hooks_1 = require("@/lib/api/hooks");
function TrustPage() {
    const [evaluating, setEvaluating] = (0, react_1.useState)(false);
    const { data: trustData, isLoading: loading, refetch } = (0, hooks_1.useTrustScore)();
    const score = trustData?.score || 0;
    const breakdown = trustData?.breakdown || null;
    const signals = trustData?.signals || null;
    async function recalculate() { setEvaluating(true); await client_1.api.post('/api/trust/evaluate', {}); await refetch(); setEvaluating(false); }
    ...
    const SIGNALS = [
        ...
        { label: 'Study Engagement', value: (breakdown?.studyEngage || 50), color: 'var(--teal)', weight: '15%' },
        { label: 'Sentinel Verified', value: (breakdown?.sentinelVerif || 70), color: 'var(--pink)', weight: '10%' },
    ];
    const ACTIONS = [
    ...
    return (<div style={{ maxWidth: 860, margin: '0 auto' }} className="animate-fade-in">
      ...
      <div className="page-hero" style={{ marginBottom: 24 }}>
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
          ...
          <button onClick={recalculate} disabled={evaluating} className={evaluating ? 'btn-ghost btn-sm' : 'btn-primary btn-sm'} style={{ flexShrink: 0 }}>
            {evaluating ? '⟳ Recalculating...' : '⟳ Recalculate Now'}
          </button>
        </div>
      </div>

      {loading ? (<div style={{ textAlign: 'center', padding: 80, color: 'var(--t3)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            ...
            <div style={{ background: 'var(--accent-light)', border: '1px solid rgba(79,70,229,0.15)', borderRadius: 16, padding: 22, boxShadow: 'var(--shadow-sm)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {ACTIONS.map((a, i) => (<div key={i} className="action-item" style={{ background: 'rgba(255,255,255,0.6)' }}>
          ...
          {/* Behavioral signals */}
          {signals && (<div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 16, padding: 22, boxShadow: 'var(--shadow-sm)' }}>
              ...
              <div className="metric-grid" style={{ margin: 0 }}>
                ...
                ].map(s => (<div key={s.label} className="metric-card" style={{ textAlign: 'center' }}>
