...
function MissionsPage() {
    ...
    const { missionStreak: streak, xp, completedMissions, onboardingAnswers, earnPins, } = cOS;
    ...
    const [extraMissions, setExtraMissions] = (0, react_1.useState)([]);
    const [generating, setGenerating] = (0, react_1.useState)(false);
    const [customSkill, setCustomSkill] = (0, react_1.useState)('');
    const [customRole, setCustomRole] = (0, react_1.useState)(onboardingAnswers?.role || 'Software Developer');
    const [generatingSkill, setGeneratingSkill] = (0, react_1.useState)(false);
    (0, react_1.useEffect)(() => {
    ...
    const handleGenerateCustomQuests = async () => {
        if (!customSkill.trim()) {
            useAppStore_1.toast.error('Skill Required', 'Please enter a skill to train.');
            return;
        }
        setGeneratingSkill(true);
        try {
            await client_1.api.post('/api/missions/generate-custom-skill', {
                targetRole: customRole,
                skill: customSkill.trim()
            });
            useAppStore_1.toast.success('Skill Quests Active! ⚡', `Learning modules for "${customSkill}" have been injected.`);
            setCustomSkill('');
            setTimeout(() => {
            ...
            useAppStore_1.toast.error('Generation Failed', err.message || 'Could not generate custom quests.');
        }
        finally {
    ...
    const { data: firestoreMissions, isLoading: fsLoading } = (0, hooks_1.useMissionsToday)();
    const { data: historyData } = (0, hooks_1.useMissionHistory)();
    ...
    (0, react_1.useEffect)(() => {
        if (typeof window !== 'undefined') {
            ...
            if (gapsParam) {
                ...
                const seeded = gapSkills.map((skill, idx) => ({
    ...
    const fsBaseMissions = (firestoreMissions || []).map((m) => ({
        ...m,
    ...
    const uniqueFs = fsBaseMissions.filter((m) => !localIds.has(m.id));
    ...
    const pastCompleted = historyMissions.filter((m) => m.status === 'completed' || m.status === 'submitted');
    ...
    const gapClosurePct = allTodayMissions.length
        ...
        : 0;
    return (<div style={{ maxWidth: 1140, margin: '0 auto' }} className="animate-fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.8fr) minmax(0, 1.2fr)', gap: 24, alignItems: 'start' }}>
        ...
        <div>
          {/* Progress bar */}
          {allTodayMissions.length > 0 && (<div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
            ...
            {tab === 'today' && (<button onClick={handleTriggerRegenerate} className="btn-ghost btn-sm" disabled={generating} style={{ fontSize: 11 }}>
                {generating ? '⟳ Re-generating...' : '⟳ Regenerate'}
              </button>)}
          </div>

          {/* Today Tab */}
          {tab === 'today' && (pending.length === 0 ? (<div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 16, padding: 32, boxShadow: 'var(--shadow-sm)' }}>
                <div className="empty-state" style={{ textAlign: 'center' }}>
                  ...
                  <link_1.default href="/dashboard" className="btn-primary btn-sm">Command Center →</link_1.default>
                </div>
              </div>) : (<div>
                <div style={{ display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
                  {Object.entries(TYPE_META).map(([type, cfg]) => {
                const count = pending.filter(m => m.type === type).length;
                return count > 0 ? (<span key={type} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 8, background: cfg.light, color: cfg.color }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {pending.map(m => (<div key={m.id} style={{ position: 'relative' }}>
          ...
          {/* History Tab */}
          {tab === 'history' && (completedMissions.length === 0 ? (<div className="empty-state" style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 16 }}>
                ...
                {allTodayMissions.filter(m => completedMissions.includes(m.id)).map(m => (<div key={m.id}>
                    <MissionCard_1.default mission={{ ...m, status: 'completed' }} onComplete={() => { }}/>
                  </div>))}

                {pastCompleted.slice(0, 5).map((m) => (<div key={m.id}>
                    <MissionCard_1.default mission={{ ...m, status: 'completed' }} onComplete={() => { }}/>
                  </div>))}
              </div>))}
        </div>
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 20, padding: 22, boxShadow: 'var(--shadow-sm)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ background: 'var(--bg3)', padding: 14, borderRadius: 12, border: '1px solid var(--border)' }}>
                ...
                {!onboardingAnswers.hasCompleted && (<link_1.default href="/career-twin" style={{ fontSize: 11, color: 'var(--accent)', textDecoration: 'none', display: 'block', marginTop: 6, fontWeight: 600 }}>
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 20, padding: 22, boxShadow: 'var(--shadow-sm)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                ...
                <input type="text" placeholder="e.g. React, Docker, CI/CD, Python" value={customSkill} onChange={(e) => setCustomSkill(e.target.value)} style={{
            ...
            color: 'var(--t1)', fontSize: 12.5, outline: 'none'
              ...
              <div>
                ...
                <input type="text" placeholder="e.g. Backend Engineer" value={customRole} onChange={(e) => setCustomRole(e.target.value)} style={{
            width: '100%', padding: '8px 12px', borderRadius: 8,
            background: 'var(--bg3)', border: '1px solid var(--border)',
            color: 'var(--t1)', fontSize: 12.5, outline: 'none'
        }}/>
              </div>
              <button type="button" onClick={handleGenerateCustomQuests} disabled={generatingSkill} className="btn-primary btn-sm" style={{ justifyContent: 'center', marginTop: 4, width: '100%' }}>
