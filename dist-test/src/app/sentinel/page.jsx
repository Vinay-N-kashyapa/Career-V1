...
function SentinelPage() {
    ...
    const [tab, setTab] = (0, react_1.useState)('access');
    ...
    const [testKey, setTestKey] = (0, react_1.useState)('');
    const [testerStep, setTesterStep] = (0, react_1.useState)('init');
    const [testMessage, setTestMessage] = (0, react_1.useState)('');
    ...
    return (<div style={{ maxWidth: 960, margin: '0 auto' }} className="animate-fade-in">
      ...
      <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 20, padding: 20, marginBottom: 24, boxShadow: 'var(--shadow-sm)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10 }}>
          {DOC_LAYERS.map((l) => (<div key={l.layer} style={{ background: 'var(--bg3)', border: `1px solid var(--border)`, borderRadius: 12, padding: 12, textAlign: 'center' }}>
      ...
      {/* Access Tab Content */}
      {tab === 'access' && (<div style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr', gap: 24 }}>
          {/* Permission Settings */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            ].map(item => (<div key={item.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                    <div>
                      <div style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--t1)' }}>{item.label}</div>
                      <div style={{ fontSize: 10, color: 'var(--t3)' }}>{item.desc}</div>
                    </div>
                    
                    <button onClick={() => togglePermission(item.key)} style={{
                    padding: '6px 12px',
                    borderRadius: 8,
                    border: 'none',
                    background: permissions[item.key] ? 'var(--accent-light)' : 'rgba(220,38,38,0.08)',
                    color: permissions[item.key] ? 'var(--accent)' : 'var(--coral)',
                    fontWeight: 700,
                    fontSize: 11.5,
                    cursor: 'pointer',
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 20, padding: 20, boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 360, overflowY: 'auto' }}>
              {logs.map(log => (<div key={log.id} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 12, padding: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
      ...
      {tab === 'tester' && (<div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 24, padding: 24, boxShadow: 'var(--shadow-sm)' }}>
          ...
          {testerStep === 'init' && (<div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 380 }} className="animate-fade-in">
              <div className="form-group">
                <label className="form-label">Enter Temporary Recruiter Key</label>
                <input className="form-input" value={testKey} onChange={e => setTestKey(e.target.value.toUpperCase())} placeholder="e.g. TEMP-9X2F"/>
              </div>
              <button onClick={() => {
                    if (!testKey.trim()) {
                ...
                }} className="btn-primary" style={{ alignSelf: 'flex-start', padding: '10px 20px', fontSize: 12.5 }}>
                Validate Recruiter Session ➔
              </button>
              {testMessage && <div style={{ fontSize: 11, color: 'var(--coral)' }}>{testMessage}</div>}
            </div>)}

          {testerStep === 'biometric' && (<div style={{ textAlign: 'center', padding: '30px 10px' }} className="animate-fade-in">
              ...
              <button onClick={() => {
                ...
                }} className="btn-primary" style={{ display: 'inline-flex', padding: '9px 18px', fontSize: 12 }}>
                Simulate Recruiter Face Scan
              </button>
            </div>)}

          {testerStep === 'scan' && (<div style={{
          ...
          {testerStep === 'view' && (<div style={{ display: 'flex', flexDirection: 'column', gap: 16 }} className="animate-fade-in">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg3)', padding: 12, borderRadius: 12, border: '1px solid var(--border)' }}>
                ...
                <button onClick={() => { setTesterStep('init'); setTestKey(''); }} className="btn-ghost btn-sm" style={{ fontSize: 11, color: 'var(--coral)' }}>
                  Revoke & Exit
                </button>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 14 }}>
                {[
                    ...
                    const allowed = permissions[doc.key];
                    return (<div key={doc.key} style={{
                            background: 'var(--bg3)',
                      ...
                      {allowed ? (<button onClick={() => alert(`Simulating file decryption & download for secure ${doc.label}`)} className="btn-primary btn-sm" style={{ width: '100%', justifyContent: 'center', padding: '4px', fontSize: 11 }}>
                          Decrypt & View
                        </button>) : (<button disabled className="btn-ghost btn-sm" style={{ width: '100%', justifyContent: 'center', padding: '4px', fontSize: 11, cursor: 'not-allowed' }}>
                          Request Permission
                        </button>)}
                    </div>);
                })}
