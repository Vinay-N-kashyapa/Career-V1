...
const PinsHistory_1 = __importDefault(require("@/components/pins/PinsHistory"));
...
function PricingPage() {
    const { data: user } = (0, hooks_1.useMe)();
    const { pins, pinHistory, addPurchasedPins } = (0, CareerOSContext_1.useCareerOS)();
    const { data: status } = (0, react_query_1.useQuery)({
        queryKey: ['payment', 'status'],
        queryFn: () => client_1.api.get('/api/payment/status'),
        enabled: !!user,
    });
    const orderMutation = (0, react_query_1.useMutation)({
        mutationFn: (planId) => client_1.api.post('/api/payment/create-order', { planId }),
        onSuccess: (data, planId) => {
            if (data.devMode) {
                useAppStore_1.toast.info('Dev Mode', 'Simulating payment...');
        ...
        onError: () => useAppStore_1.toast.error('Payment Error', 'Could not initiate payment. Please try again.'),
    });
    const verifyMutation = (0, react_query_1.useMutation)({
        mutationFn: (data) => client_1.api.post('/api/payment/verify', data),
        onSuccess: (data) => { useAppStore_1.toast.success('🎉 Plan Active!', data.message); window.location.reload(); },
        onError: () => useAppStore_1.toast.error('Verification Failed', 'Contact support with your order ID.'),
    });
    const packMutation = (0, react_query_1.useMutation)({
        mutationFn: (pack) => client_1.api.post('/api/payment/create-order', { planId: pack.id, amount: pack.priceNum }),
        onSuccess: (data, pack) => {
            if (data.devMode) {
        ...
        onError: () => useAppStore_1.toast.error('Payment Error', 'Try again.'),
    });
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 18, overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {PIN_PACKS.map(pack => (<div key={pack.id} style={{
                ...
                borderRadius: 12, position: 'relative',
                  ...
                  {pack.highlight && (<div style={{ position: 'absolute', top: -8, right: 12, background: 'var(--amber)', color: '#000', fontSize: 9, fontWeight: 800, padding: '2px 8px', borderRadius: 10, letterSpacing: '0.5px' }}>
                      BEST VALUE
                    </div>)}
                  ...
                  <button onClick={() => packMutation.mutate(pack)} disabled={packMutation.isPending} style={{
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 18, overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <div style={{ padding: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {EARN_WAYS.map(w => (<link_1.default key={w.label} href={w.href} style={{ textDecoration: 'none' }}>
                  <div style={{
                display: 'flex', alignItems: 'center', gap: 8,
                padding: '10px 12px', borderRadius: 10,
                background: 'var(--bg3)', border: '1px solid var(--border)',
                transition: 'all 0.15s', cursor: 'pointer',
            }} onMouseEnter={e => { e.currentTarget.style.borderColor = w.color; e.currentTarget.style.background = 'var(--bg2)'; }} onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.background = 'var(--bg3)'; }}>
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 18, overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <table className="data-table" style={{ margin: 0 }}>
              ...
              <tbody>
                {Object.entries(CareerOSContext_1.PIN_COSTS).map(([key, meta]) => {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 18, overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ padding: '16px 18px', border: `2px solid ${currentTier === 'free' ? 'var(--border2)' : 'var(--border)'}`, borderRadius: 14, background: currentTier === 'free' ? 'var(--bg3)' : 'transparent' }}>
                ...
                {['100 starter pins', '3 AI interviews/mo', '2 resume uploads/mo', 'Basic Career DNA', 'Full mission system'].map(f => (<div key={f} style={{ fontSize: 12.5, color: 'var(--t2)', padding: '3px 0', display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <div style={{ padding: '16px 18px', border: `2px solid ${isPro ? 'var(--accent)' : 'rgba(79,70,229,0.3)'}`, borderRadius: 14, background: 'linear-gradient(135deg, rgba(79,70,229,0.08), rgba(124,58,237,0.06))', position: 'relative' }}>
                ...
                {['200 pins/month included', 'Unlimited AI interviews', 'Unlimited resume uploads', 'Full Career Twin simulation', 'Priority evaluation queue', 'All avatar coaching modes'].map(f => (<div key={f} style={{ fontSize: 12.5, color: 'var(--t1)', padding: '3px 0', display: 'flex', gap: 8, alignItems: 'center' }}>
                    <span style={{ color: 'var(--accent)', fontSize: 10 }}>✓</span>{f}
                  </div>))}
                {!isPro && (<button onClick={() => orderMutation.mutate('pro')} disabled={orderMutation.isPending || verifyMutation.isPending} className="btn-primary" style={{ width: '100%', justifyContent: 'center', marginTop: 14 }}>
              ...
              <div style={{ padding: '14px 18px', border: '1px solid var(--border)', borderRadius: 14 }}>
                ...
                {['Everything in Pro', 'Bulk student management', 'Placement analytics', 'Attendance system', 'Dedicated support'].map(f => (<div key={f} style={{ fontSize: 12, color: 'var(--t2)', padding: '2px 0', display: 'flex', gap: 8 }}>
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 18, overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <div style={{ padding: 16 }}>
              <PinsHistory_1.default limit={10}/>
            </div>
          </div>
