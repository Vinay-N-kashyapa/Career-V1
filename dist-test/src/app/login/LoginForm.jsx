...
function getRedirectPath(email, role) {
...
function LoginForm() {
    ...
    // Mode state
    const [mode, setMode] = (0, react_1.useState)('password');
    // Credentials State
    ...
    // QR Session State
    const [qrToken, setQrToken] = (0, react_1.useState)(null);
    const [qrUrl, setQrUrl] = (0, react_1.useState)(null);
    const [qrStatus, setQrStatus] = (0, react_1.useState)('loading');
    const [timeLeft, setTimeLeft] = (0, react_1.useState)(300);
    const [qrMessage, setQrMessage] = (0, react_1.useState)('');
    const [simulating, setSimulating] = (0, react_1.useState)(false);
    const [isLocalSimulation, setIsLocalSimulation] = (0, react_1.useState)(false);
    const unsubRef = (0, react_1.useRef)(null);
    // Generate QR Session
    const createQRSession = (0, react_1.useCallback)(async () => {
        if (unsubRef.current) {
            unsubRef.current();
                ...
                if (row.status === 'scanned') {
                    ...
                    if (row.email && row.password) {
                        ...
                        try {
                            ...
                            await supabaseClient_1.supabase.from('qr_login_sessions').delete().eq('id', token);
    ...
    (0, react_1.useEffect)(() => {
        if (mode !== 'qr' || (qrStatus !== 'ready' && qrStatus !== 'scanned'))
            return;
        const t = setInterval(() => {
            setTimeLeft(l => {
                if (l <= 1) {
    ...
    return (<div style={{ width: '100%' }}>
      ...
      <div style={{
        ...
        <button id="btn-tab-pwd" type="button" onClick={() => { setMode('password'); setError(''); }} style={{
            padding: '8px 12px',
            borderRadius: 8,
            fontSize: 12,
            fontWeight: 600,
        ...
        <button id="btn-tab-qr" type="button" onClick={() => { setMode('qr'); setError(''); }} style={{
            padding: '8px 12px',
            borderRadius: 8,
            fontSize: 12,
            fontWeight: 600,
      ...
      {mode === 'password' && (<form onSubmit={submitPasswordForm} className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="form-group" style={{ marginBottom: 0 }}>
            <label className="form-label" style={{ fontSize: 11 }}>Username / Email</label>
            <input id="input-username" type="text" placeholder="Enter your username or email" value={form.username} onChange={e => setForm(f => ({ ...f, username: e.target.value }))} className="form-input" autoComplete="username" required autoFocus/>
          </div>

          <div className="form-group" style={{ marginBottom: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              <label className="form-label" style={{ marginBottom: 0, fontSize: 11 }}>Password</label>
              <link_1.default id="link-forgot-pwd" href="/reset-password" style={{ fontSize: 11, color: 'var(--accent)', textDecoration: 'none' }}>
                Forgot password?
              </link_1.default>
            </div>
            <div className="input-group">
              <input id="input-password" type={showPwd ? 'text' : 'password'} placeholder="••••••••" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} className="form-input" style={{ paddingRight: 40 }} autoComplete="current-password" required/>
              <button id="btn-toggle-pwd" type="button" onClick={() => setShowPwd(v => !v)} style={{
                position: 'absolute',
                right: 12,
                top: '50%',
          ...
          <div style={{
            ...
            ].map(demo => (<button id={`btn-shortcut-${demo.label.toLowerCase()}`} key={demo.label} type="button" onClick={() => {
          ...
          {error && (<div className="alert alert-danger" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: 12, borderRadius: 10, fontSize: 12, marginBottom: 0 }}>
              <span>⚠️</span>
              {error}
            </div>)}

          <button id="btn-submit-pwd" type="submit" className="btn-primary" style={{ width: '100%', justifyContent: 'center', padding: '12px', fontSize: 13.5, fontWeight: 700, marginTop: 4 }} disabled={loading}>
            {loading ? '⏳ Logging in...' : 'Sign In →'}
          </button>
        </form>)}

      {/* MODE 2: QR LOGIN SCAN */}
      {mode === 'qr' && (<div style={{ textAlign: 'center' }} className="animate-fade-in">
          <div style={{
                width: 220,
                height: 220,
                margin: '0 auto 16px',
                borderRadius: 16,
                overflow: 'hidden',
                border: `2px solid ${qrStatus === 'confirmed'
                    ? 'var(--green)'
                    : qrStatus === 'scanned'
                        ? 'var(--accent)'
            ...
            {qrStatus === 'loading' && (<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, color: 'var(--t3)' }}>
            ...
            {qrStatus === 'expired' && (<div id="btn-refresh-qr" onClick={createQRSession} style={{
                    width: '100%',
                    height: '100%',
                    display: 'flex',
                    flexDirection: 'column',
          ...
          {(qrStatus === 'ready' || qrStatus === 'scanned') && (<button id="btn-simulate-scan" type="button" onClick={handleSimulateMobileScan} disabled={simulating} className="btn-primary" style={{
                    width: '100%',
                    padding: '10px',
                    fontSize: 12,
                    fontWeight: 600,
                    ...
                    justifyContent: 'center',
                    gap: 6
                }} onMouseOver={(e) => e.currentTarget.style.background = 'rgba(79, 70, 229, 0.25)'} onMouseOut={(e) => e.currentTarget.style.background = 'rgba(79, 70, 229, 0.15)'}>
              {simulating ? '⏳ Scanning Biometrics...' : '📱 Simulate Biometric Phone Scan'}
            </button>)}

          {/* Simulated credentials sync fields */}
          {qrStatus === 'ready' && (<div style={{
                    padding: 12,
                    background: 'rgba(15, 23, 42, 0.4)',
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <input id="input-sync-email" placeholder="Sync Email" value={form.username} onChange={e => setForm(f => ({ ...f, username: e.target.value }))} className="form-input" style={{ fontSize: 11, padding: '6px 10px' }}/>
                <input id="input-sync-pwd" type="password" placeholder="Sync Password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} className="form-input" style={{ fontSize: 11, padding: '6px 10px' }}/>
              </div>
            </div>)}

          {qrStatus === 'expired' && (<button id="btn-new-qr" onClick={createQRSession} className="btn-primary" style={{ width: '100%', padding: '12px', fontSize: 13, fontWeight: 700 }}>
              Generate New QR
            </button>)}
        </div>)}
    </div>);
