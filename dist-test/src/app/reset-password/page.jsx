...
function ResetForm() {
    ...
    async function requestReset(e) {
        ...
        setError('');
        try {
            await client_1.api.post('/api/auth/forgot-password', { email });
            setSent(true);
        }
        catch {
    ...
    async function doReset(e) {
        ...
        setError('');
        try {
            await client_1.api.post('/api/auth/reset-password', { token, password, confirmPassword: confirm });
            setSuccess(true);
    ...
    return (<div className="auth-page">
      <div className="auth-card animate-fade-in">
        ...
        {sent && !token && (<div className="alert alert-success" style={{ marginBottom: 16 }}>
            <span>✓</span> If that account exists, a reset link has been sent to {email}. Check your inbox.
          </div>)}

        {error && (<div className="alert alert-danger" style={{ marginBottom: 14 }}>
            <span>⚠</span> {error}
          </div>)}

        {/* Request reset form */}
        {!token && !sent && (<form onSubmit={requestReset}>
            ...
            <button type="submit" className="btn-primary" style={{ width: '100%', justifyContent: 'center', padding: '11px', fontSize: 14 }} disabled={loading}>
              {loading ? '⏳ Sending...' : 'Send Reset Link →'}
            </button>
          </form>)}

        {/* New password form */}
        {token && !success && (<form onSubmit={doReset}>
            <div className="form-group">
              <label className="form-label">New Password</label>
              <input className="form-input" type="password" placeholder="min 8 characters" value={password} onChange={e => setPassword(e.target.value)} required minLength={8} autoFocus/>
            </div>
            <div className="form-group">
              <label className="form-label">Confirm Password</label>
              <input className="form-input" type="password" placeholder="repeat new password" value={confirm} onChange={e => setConfirm(e.target.value)} required/>
            </div>
            {/* Strength indicator */}
            {password && (<div style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
                  {[1, 2, 3, 4].map(i => (<div key={i} style={{ flex: 1, height: 3, borderRadius: 3, background: password.length >= i * 3 ? (i <= 2 ? 'var(--amber)' : i === 3 ? 'var(--teal)' : 'var(--green)') : 'var(--bg3)', transition: 'background 0.3s' }}/>))}
                </div>
        ...
        <div style={{ textAlign: 'center', marginTop: 18, fontSize: 12.5, color: 'var(--t3)' }}>
          <link_1.default href="/login" style={{ color: 'var(--accent)', fontWeight: 500, textDecoration: 'none' }}>
            ← Back to Sign In
          </link_1.default>
        </div>
      </div>
...
function ResetPasswordPage() {
    return (<react_1.Suspense fallback={<div className="auth-page"><div className="auth-card">Loading...</div></div>}>
      <ResetForm />
    </react_1.Suspense>);
}
