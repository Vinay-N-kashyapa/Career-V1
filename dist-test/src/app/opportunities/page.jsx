...
exports.default = OpportunitiesPage;
...
function ApplyButton({ opportunityId, title }) {
    const applyMutation = (0, hooks_1.useApply)();
    const [applied, setApplied] = (0, react_1.useState)(false);
    if (applied || applyMutation.isSuccess) {
        return (<span style={{ fontSize: 11, fontFamily: 'var(--font-mono)', color: 'var(--green)', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 4 }}>
        ✓ Applied
      </span>);
    }
    return (<button onClick={() => { applyMutation.mutate({ opportunityId }); setApplied(true); }} disabled={applyMutation.isPending} style={{
...
function OpportunitiesPage() {
    ...
    const [opps, setOpps] = (0, react_1.useState)([]);
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [filter, setFilter] = (0, react_1.useState)('all');
    const [jd, setJD] = (0, react_1.useState)('');
    const [matching, setMatching] = (0, react_1.useState)(false);
    const [matchResult, setMatchResult] = (0, react_1.useState)(null);
    // AI Interview Coach section states
    const [activeRound, setActiveRound] = (0, react_1.useState)('none');
    const [interviewStarted, setInterviewStarted] = (0, react_1.useState)(false);
    const [mockQuestion, setMockQuestion] = (0, react_1.useState)('');
    const [mockAnswer, setMockAnswer] = (0, react_1.useState)('');
    const [interviewResult, setInterviewResult] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
        fetchOpps();
    ...
    async function fetchOpps() {
        ...
        try {
            ...
            if (onboardingAnswers.role)
                params.targetRole = onboardingAnswers.role;
            const qs = Object.keys(params).length ? '?' + new URLSearchParams(params).toString() : '';
            const d = await client_1.api.get(`/api/opportunities${qs}`);
    ...
    async function matchJD() {
        ...
        setMatching(true);
        try {
            const d = await client_1.api.post('/api/opportunities/match', { jd });
            const result = (d.match || {});
            setMatchResult({
    ...
    function launchMockInterview(round) {
    ...
    return (<div style={{ maxWidth: 1040, margin: '0 auto' }} className="animate-fade-in">
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        {onboardingAnswers.hasCompleted && (<div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'var(--teal)', background: 'rgba(20,184,166,0.1)', padding: '4px 10px', borderRadius: 8, border: '1px solid rgba(20,184,166,0.2)', fontSize: 11.5, fontFamily: 'var(--font-mono)', fontWeight: 700, marginTop: 8 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 20, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 20, padding: 22, boxShadow: 'var(--shadow-sm)' }}>
            ...
            <textarea value={jd} onChange={e => setJD(e.target.value)} placeholder="Paste any corporate SDE or ML job description here to check your exact matching vs. missing skill gaps..." rows={4} style={{ width: '100%', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 12px', color: 'var(--t1)', fontSize: 12, fontFamily: 'var(--font-body)', outline: 'none', resize: 'vertical', lineHeight: 1.6, marginBottom: 10 }}/>
            ...
            {matchResult && (() => {
            const m = matchResult;
            return (<div style={{ marginTop: 16, padding: '14px 16px', background: 'var(--bg3)', borderRadius: 12, border: '1px solid var(--border)' }} className="animate-fade-in">
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', borderTop: '1px solid var(--border)', paddingTop: 12 }}>
                    <link_1.default href={`/missions?gaps=${m.missing_skills?.join(',')}`} className="btn-primary btn-sm" style={{ textDecoration: 'none', fontSize: 11, padding: '6px 12px' }}>
                      ⚡ Generate Missions from Gaps
                    </link_1.default>
                    <link_1.default href="/career-twin" className="btn-ghost btn-sm" style={{ textDecoration: 'none', fontSize: 11, padding: '6px 12px' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            {['all', 'job', 'internship', 'scholarship', 'competition'].map(t => (<button key={t} onClick={() => setFilter(t)} style={{ padding: '5px 14px', borderRadius: 20, border: `1px solid ${filter === t ? 'var(--accent)' : 'var(--border)'}`, background: filter === t ? 'rgba(79,70,229,0.1)' : 'transparent', color: filter === t ? '#8b8bf5' : 'var(--t2)', cursor: 'pointer', fontSize: 12 }}>
          ...
          {/* Opportunity Feed */}
          {loading ? (<div style={{ textAlign: 'center', padding: 48, color: 'var(--t3)' }}>Locating active job fits...</div>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {opps.map((o) => (<div key={o.id} style={{ background: 'var(--bg2)', border: `1.5px solid ${o.priority === 'urgent' ? 'rgba(239,68,68,0.2)' : 'var(--border)'}`, borderRadius: 16, padding: '18px 22px', display: 'flex', gap: 16, alignItems: 'flex-start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, rgba(79,70,229,0.04), rgba(124,58,237,0.02))', border: '2px solid rgba(79,70,229,0.15)', borderRadius: 20, padding: 22, boxShadow: 'var(--shadow-sm)' }}>
            ...
            {/* Launch Panel */}
            {!interviewStarted && !interviewResult && (<div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {interviewStarted && (<div style={{ display: 'flex', flexDirection: 'column', gap: 10 }} className="animate-fade-in">
                ...
                <textarea value={mockAnswer} onChange={e => setMockAnswer(e.target.value)} placeholder="Record or type your response using the STAR method..." rows={4} style={{ width: '100%', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 10, padding: 10, color: 'var(--t1)', fontSize: 12, outline: 'none', resize: 'vertical' }}/>
            ...
            {/* Evaluation Result */}
            {interviewResult && (<div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 14, padding: 14 }} className="animate-fade-in">
        ...
        ].map((round, idx) => (<div key={idx} style={{ display: 'flex', alignItems: 'center', fontSize: 12, justifyContent: 'space-between' }}>
                  <div>
