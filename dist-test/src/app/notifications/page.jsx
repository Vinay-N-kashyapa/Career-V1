...
function NotificationsPage() {
    ...
    const { data: notifData, isLoading } = (0, hooks_1.useNotifications)();
    const markReadMutation = (0, hooks_1.useMarkRead)();
    const router = (0, navigation_1.useRouter)();
    const notifs = notifData || [];
    async function markAllRead() { await client_1.api.post('/api/notifications/mark-all-read', {}); }
    function getActionUrl(n) { return n.action_url || SOURCE_FALLBACK[n.source] || null; }
    ...
    return (<div style={{ maxWidth: 720, margin: '0 auto' }} className="animate-fade-in">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, gap: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
          <button onClick={() => setFilter(f => f === 'all' ? 'unread' : 'all')} className="btn-ghost btn-sm" style={{ borderColor: filter === 'unread' ? 'var(--accent)' : undefined, color: filter === 'unread' ? 'var(--accent)' : undefined }}>
            {filter === 'all' ? '🔵 Unread Only' : '📋 Show All'}
          </button>
          {unread > 0 && (<button onClick={markAllRead} className="btn-ghost btn-sm">✓ Mark All Read</button>)}
        </div>
      </div>

      {isLoading ? (<div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {[...Array(6)].map((_, i) => <div key={i} className="skeleton" style={{ height: 72, borderRadius: 12 }}/>)}
        </div>) : displayed.length === 0 ? (<div className="empty-state-enhanced">
          ...
          {displayed.map(n => {
                ...
                return (<div key={n.id} onClick={() => handleClick(n)} role={clickable ? 'button' : undefined} tabIndex={clickable ? 0 : undefined} onKeyDown={e => { if (clickable && (e.key === 'Enter' || e.key === ' '))
                    handleClick(n); }} style={{
                    ...
                    e.currentTarget.style.borderColor = meta.color; }} onMouseLeave={e => { e.currentTarget.style.borderColor = n.is_read ? 'var(--border)' : 'var(--border2)'; }}>
                {/* Icon */}
                <div style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    {actionUrl && (<span style={{ fontSize: 10, color: meta.color, fontFamily: 'var(--font-mono)', fontWeight: 600, marginLeft: 'auto' }}>
                        → {actionUrl}
                      </span>)}
                  </div>
                </div>
