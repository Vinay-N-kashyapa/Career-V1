...
exports.ATS_WEIGHTS = void 0;
exports.scoreResume = scoreResume;
exports.default = ATSBreakdown;
...
function scoreResume(data) {
    ...
    if (data.fullName?.trim()) {
        contact += 4;
        strengths.push('Full name present');
    }
    else
    ...
    if (data.address?.trim()) {
        ...
        tips.push('Add your location / city');
    ...
    if (summary.length > 0) {
        ...
        if (summary.length >= 100) {
            ...
            tips.push('Expand your summary to at least 100 characters');
        }
        ...
        if (kwHits >= 3)
            strengths.push('Summary uses strong action keywords');
        else
            tips.push('Use more action words in your summary');
    }
    else {
    ...
    if (exps.length === 0) {
        ...
        if (exps.length >= 2)
            strengths.push(`${exps.length} experience entries found`);
        let hasDesc = 0;
        let hasDate = 0;
        let kwTotal = 0;
        exps.forEach((e) => {
            if ((e.description?.trim().length || 0) > 50)
                hasDesc++;
            if (e.startDate)
                hasDate++;
            kwTotal += ATS_KEYWORDS.filter((k) => (e.description || '').toLowerCase().includes(k)).length;
        });
        if (hasDesc > 0) {
            expS += 25;
            strengths.push('Experience includes descriptions');
        }
        else {
            tips.push('Add descriptions to your experience entries');
        }
        if (hasDate > 0) {
            expS += 15;
        }
        else {
            tips.push('Add start/end dates to your experience');
        }
        if (kwTotal >= 5) {
            expS += 20;
            strengths.push('Experience uses strong action verbs');
        }
        else {
            tips.push('Use more action verbs in experience descriptions');
        }
    }
    ...
    if (edus.length === 0) {
        ...
        if (edus.some((e) => e.institution?.trim()))
            eduS += 25;
        else
            tips.push('Add institution names');
        if (edus.some((e) => e.year?.trim()))
            eduS += 25;
        else
            tips.push('Add graduation year');
    }
    ...
    if (!allSkills) {
        ...
        if (skillCount >= 5)
            ...
            tips.push('Add more skills — aim for at least 5');
        if (skills.technical?.trim()) {
            skillS += 20;
            strengths.push('Technical skills section filled');
        }
        else
            tips.push('Add technical skills');
        if (skills.professional?.trim())
            skillS += 10;
        else
            tips.push('Add professional skills');
        if (skills.personal?.trim())
            skillS += 10;
    }
    ...
    if (projs.length === 0) {
        ...
        if (projs.some((p) => p.description?.trim()))
            projS += 30;
        else
            tips.push('Add descriptions to your projects');
        if (projs.some((p) => p.technologies?.trim()))
            projS += 20;
        else
            tips.push('Add technologies used in your projects');
    }
    ...
    if (certs.length > 0) {
        extrasS += 50;
        strengths.push(`${certs.length} certification(s) added`);
    }
    else {
        tips.push('Add certifications to stand out');
    }
    if (langs.length > 0)
        extrasS += 30;
    if (data.declaration?.trim())
        extrasS += 20;
    scores.extras = Math.min(100, extrasS);
    // ── Weighted overall ──────────────────────────────────────────────
    const overall = Math.round(Object.keys(exports.ATS_WEIGHTS).reduce((sum, key) => sum + (scores[key] * exports.ATS_WEIGHTS[key].weight) / 100, 0));
    return { overall, scores, tips, strengths };
}
// ── Display ─────────────────────────────────────────────────────────
const tierColor = (v) => v >= 80 ? 'var(--green, #10b981)' : v >= 60 ? 'var(--amber, #f59e0b)' : 'var(--coral, #ef4444)';
const tierLabel = (v) => v >= 80 ? 'Excellent ✨' : v >= 60 ? 'Good 👍' : 'Needs Work 🔧';
const tierBgLight = (v) => v >= 80 ? 'rgba(16,185,129,0.12)' : v >= 60 ? 'rgba(245,158,11,0.12)' : 'rgba(239,68,68,0.12)';
function ScoreRing({ score }) {
    const color = tierColor(score);
    const r = 54;
    const circ = 2 * Math.PI * r;
    const dash = (score / 100) * circ;
    return (<div style={{ position: 'relative', width: 140, height: 140, flexShrink: 0 }}>
...
function ATSBreakdown({ resumeData, compact = false }) {
    const result = (0, react_1.useMemo)(() => scoreResume(resumeData), [resumeData]);
    ...
    return (<div style={S.wrapper}>
      ...
      <div style={S.section}>
        ...
        {Object.keys(exports.ATS_WEIGHTS).map((key) => (<charts_1.FunnelBar key={key} label={exports.ATS_WEIGHTS[key].label} value={result.scores[key]} max={100} color={tierColor(result.scores[key])} labelWidth={100}/>))}
      </div>

      {!compact && (<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 12, marginBottom: 12 }}>
          <div style={{ ...S.feedCard, borderLeftColor: 'var(--green, #10b981)' }}>
            <h4 style={{ ...S.feedTitle, color: 'var(--green-dark, #065f46)' }}>
              ✅ Strengths ({result.strengths.length})
            </h4>
            {result.strengths.length === 0 ? (<p style={{ color: 'var(--t3, #9ca3af)', fontSize: 12 }}>Complete more sections to see strengths</p>) : (<ul style={S.feedList}>
          ...
          <div style={{ ...S.feedCard, borderLeftColor: 'var(--amber, #f59e0b)' }}>
            ...
            {result.tips.length === 0 ? (<p style={{ color: 'var(--green, #10b981)', fontWeight: 600, fontSize: 12 }}>🎉 No major issues found!</p>) : (<ul style={S.feedList}>
...
const S = {
    ...
    title: { fontSize: 19, fontWeight: 800, color: 'var(--t1, #1f2937)', marginBottom: 4 },
    sub: { color: 'var(--t3, #9ca3af)', fontSize: 12 },
    scoreSection: {
    ...
    sectionTitle: { fontSize: 14, fontWeight: 700, color: 'var(--t1, #1f2937)', marginBottom: 14 },
    feedCard: {
        background: 'var(--bg2, white)',
        border: '1px solid var(--border, #e5e7eb)',
    ...
    feedTitle: { fontSize: 13, fontWeight: 700, marginBottom: 10 },
    feedList: { paddingLeft: 18, color: 'var(--t2, #4b5563)', fontSize: 12, lineHeight: 1.7, margin: 0 },
};
