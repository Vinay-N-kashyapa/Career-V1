...
function ResumeUpload() {
    const { vaultItems, setResumeGenerated, generateFusedRoadmap } = (0, CareerOSContext_1.useCareerOS)();
    const [dragging, setDragging] = (0, react_1.useState)(false);
    const [uploading, setUploading] = (0, react_1.useState)(false);
    const [result, setResult] = (0, react_1.useState)(null);
    const [error, setError] = (0, react_1.useState)('');
    const [improving, setImproving] = (0, react_1.useState)(false);
    const [improved, setImproved] = (0, react_1.useState)(null);
    const [showVaultModal, setShowVaultModal] = (0, react_1.useState)(false);
    ...
    const handleImportFromVault = async (itemId) => {
        setUploading(true);
        setError('');
        setResult(null);
        setImproved(null);
        try {
            const res = await client_1.api.post('/api/resume/generate-from-vault', { itemId });
            if (res.analysis) {
                setResult({
                ...
                useAppStore_1.toast.success('Secure Vault Document Synced!', 'AI analyzed details, computed ATS scores, and updated roadmap quests.');
            }
            else {
    ...
    async function improve() {
        ...
        setImproving(true);
        try {
            const data = await client_1.api.post(`/api/resume/${result.resumeId}/improve`, {});
            // The backend wraps the data in either { improvements: ... } or returns it directly.
            setImproved(data?.improvements ?? data);
        }
        catch {
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {!result && (<div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop} onClick={() => fileRef.current?.click()} style={{
            ...
            {uploading && (<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, color: 'var(--accent)' }}>
            ...
            <input ref={fileRef} type="file" accept=".pdf,.docx,.doc,.txt" style={{ display: 'none' }} onChange={(e) => e.target.files?.[0] && uploadFile(e.target.files[0])}/>
          </div>

          <button type="button" onClick={(e) => { e.stopPropagation(); setShowVaultModal(true); }} disabled={uploading} style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 300, overflowY: 'auto' }}>
              {vaultItems.length === 0 ? ([
                ...
                return (<div key={item.id} onClick={() => setSelectedVaultItemId(item.id)} style={{
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 10 }}>
              <button type="button" className="btn-ghost" onClick={() => setShowVaultModal(false)} style={{ padding: '8px 16px', fontSize: 13 }}>
                Cancel
              </button>
              <button type="button" className="btn-primary" onClick={() => handleImportFromVault(selectedVaultItemId)} style={{ padding: '8px 20px', fontSize: 13 }}>
                Compile Document
              </button>
            </div>
          </div>
      ...
      {a && (<>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
            {[
                ...
                {s.isNum && (<div style={{ height: 4, background: 'var(--bg3)', borderRadius: 4, marginTop: 12, overflow: 'hidden', border: '1px solid var(--border)' }}>
                    <div style={{ height: '100%', width: `${s.val}%`, background: `var(--${s.color})`, borderRadius: 4 }}/>
                  </div>)}
              </div>))}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <div style={cardStyle}>
              ...
              {(a.strengths || []).map((s, i) => (<div key={i} style={listRow}>
            ...
            <div style={cardStyle}>
              ...
              {(a.improvement_suggestions || []).map((s, i) => (<div key={i} style={listRow}>
          ...
          {(a.keyword_gaps || []).length > 0 && (<div style={{ background: 'var(--coral-light)', border: '1px solid #fecaca', borderRadius: 'var(--radius-xl)', padding: '18px 20px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                {a.keyword_gaps.map((k) => (<a key={k} href={`/learn?topic=${encodeURIComponent(k)}`} style={{ textDecoration: 'none' }} title={`Study ${k} in Learn`}>
          ...
          {improved && (<div style={{ background: 'var(--accent-light)', border: '1px solid #c7d2fe', borderRadius: 'var(--radius-xl)', padding: 22 }}>
              ...
              {improved.rewritten_summary && (<div style={{ marginBottom: 16 }}>
              ...
              {improved.projected_ats_score && (<div style={{ fontSize: 13, color: 'var(--green)', fontWeight: 600, fontFamily: 'var(--font-mono)' }}>
          ...
          <div style={{
            ...
            {(a.keyword_gaps || []).length > 0 && (<NextStepCard_1.default eyebrow="Step 1 of 2" title="Close your top skill gap" description={`You're missing ${a.keyword_gaps.length} keyword${a.keyword_gaps.length > 1 ? 's' : ''} that recruiters search for. Study the most common one first.`} href={`/learn?topic=${encodeURIComponent(a.keyword_gaps[0])}`} ctaLabel={`Study ${a.keyword_gaps[0]} →`} icon="📚" color="var(--coral)"/>)}
            <NextStepCard_1.default eyebrow={(a.keyword_gaps || []).length > 0 ? 'Step 2 of 2' : 'Next step'} title="See opportunities that match" description={`Your ATS profile is ready. ${Math.round(a.ats_score) >= 70 ? "You're well-positioned for matches." : 'Match accuracy improves as your ATS score climbs.'}`} href="/opportunities" ctaLabel="Browse →" icon="🎯" color="var(--teal)"/>
          </div>
        </>)}
    </div>);
}
