...
const ATSBreakdown_1 = __importDefault(require("./ATSBreakdown"));
const ResumeForm_types_1 = require("./ResumeForm.types");
...
function calcCompletion(d) {
    ...
    // Summary
    total += 1;
    if (d.summary?.trim().length >= 50)
        done++;
    // Experience (at least one filled)
    total += 1;
    if (d.experiences.some((e) => e.role && e.company))
        done++;
    // Education
    total += 1;
    ...
    // Skills (any of three)
    total += 1;
    if (d.skills.technical || d.skills.professional || d.skills.personal)
        done++;
    // Projects (optional)
    total += 1;
    ...
    total += 1;
    if (d.certificates.some((c) => c.name))
        done++;
    // Languages (optional)
    total += 1;
    if (d.languages.some((l) => l.language))
        done++;
    return Math.round((done / total) * 100);
}
function ResumeForm({ initialData, onSave, showATS = true }) {
    const [data, setData] = (0, react_1.useState)(initialData ?? (0, ResumeForm_types_1.emptyFormData)());
    const [active, setActive] = (0, react_1.useState)('basic');
    ...
    (0, react_1.useEffect)(() => { if (initialData)
        setData(initialData); }, [initialData]);
    const completion = (0, react_1.useMemo)(() => calcCompletion(data), [data]);
    // ── Mutators ──────────────────────────────────────────────────────
    const set = (field, value) => setData((d) => ({ ...d, [field]: value }));
    const setSkill = (k, v) => setData((d) => ({ ...d, skills: { ...d.skills, [k]: v } }));
    const updateArr = (field, id, patch) => {
        setData((d) => ({
            ...d,
            [field]: d[field].map((item) => (item.id === id ? { ...item, ...patch } : item)),
        }));
    };
    const addArr = (field) => {
        const newItem = field === 'experiences' ? (0, ResumeForm_types_1.emptyExperience)() :
            field === 'education' ? (0, ResumeForm_types_1.emptyEducation)() :
                field === 'certificates' ? (0, ResumeForm_types_1.emptyCertificate)() :
                    field === 'projects' ? (0, ResumeForm_types_1.emptyProject)() :
                        (0, ResumeForm_types_1.emptyLanguage)();
        setData((d) => ({ ...d, [field]: [...d[field], newItem] }));
    };
    const delArr = (field, id) => {
        setData((d) => ({ ...d, [field]: d[field].filter((x) => x.id !== id) }));
    };
    // ── Save ──────────────────────────────────────────────────────────
    ...
    // ── Render ────────────────────────────────────────────────────────
    return (<div style={{ display: 'grid', gridTemplateColumns: showATS ? '180px 1fr 360px' : '180px 1fr', gap: 20 }}>
      {/* ─ Section nav (left) ─ */}
      <aside style={{ position: 'sticky', top: 24, alignSelf: 'flex-start' }}>
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {ResumeForm_types_1.FORM_SECTIONS.map((s) => (<button key={s.id} type="button" onClick={() => setActive(s.id)} style={{
                ...S.navBtn,
        ...
        {savedAt && !saving && (<div style={{ fontSize: 10, color: 'var(--t3)', textAlign: 'center', marginTop: 6, fontFamily: 'var(--font-mono)' }}>
      ...
      <main style={S.formCard}>
        ...
        {active === 'experience' && <ExperienceSection items={data.experiences} update={(id, p) => updateArr('experiences', id, p)} add={() => addArr('experiences')} del={(id) => delArr('experiences', id)}/>}
        {active === 'education' && <EducationSection items={data.education} update={(id, p) => updateArr('education', id, p)} add={() => addArr('education')} del={(id) => delArr('education', id)}/>}
        {active === 'skills' && <SkillsSection skills={data.skills} setSkill={setSkill}/>}
        {active === 'projects' && <ProjectsSection items={data.projects} update={(id, p) => updateArr('projects', id, p)} add={() => addArr('projects')} del={(id) => delArr('projects', id)}/>}
        {active === 'certificates' && <CertsSection items={data.certificates} update={(id, p) => updateArr('certificates', id, p)} add={() => addArr('certificates')} del={(id) => delArr('certificates', id)}/>}
        {active === 'languages' && <LanguagesSection items={data.languages} update={(id, p) => updateArr('languages', id, p)} add={() => addArr('languages')} del={(id) => delArr('languages', id)}/>}
        {active === 'declaration' && <DeclarationSection data={data} set={set}/>}
      </main>

      {/* ─ ATS sidebar (right) ─ */}
      {showATS && (<aside style={{ position: 'sticky', top: 24, alignSelf: 'flex-start' }}>
          <ATSBreakdown_1.default resumeData={data} compact/>
        </aside>)}
    </div>);
}
// ════════════════════════════════════════════════════════════════════
...
function ItemCard({ title, onDelete, children }) {
    return (<div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 16, marginBottom: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--t1)' }}>{title}</div>
        {onDelete && (<button type="button" onClick={onDelete} style={{ background: 'transparent', border: 'none', color: 'var(--coral)', cursor: 'pointer', fontSize: 12, padding: '4px 8px', borderRadius: 4 }}>
            ✕ Remove
          </button>)}
      </div>
      {children}
...
function AddButton({ onClick, label }) {
    return (<button type="button" onClick={onClick} className="btn-ghost" style={{ width: '100%' }}>
      + {label}
    </button>);
}
function BasicInfoSection({ data, set }) {
    return (<>
...
// ── Summary ─────────────────────────────────────────────────────────
function SummarySection({ data, set }) {
    return (<>
      ...
      <Field label="Summary" hint="Aim for 100–250 chars · use action verbs (built, led, designed, increased)">
        <textarea className="input" rows={6} value={data.summary} onChange={(e) => set('summary', e.target.value)} placeholder="Full-stack engineer with 3 years building production React + Node systems. Shipped..."/>
      </Field>
...
// ── Experience ──────────────────────────────────────────────────────
function ExperienceSection({ items, update, add, del }) {
    return (<>
      <FormHeader icon="💼" title="Work Experience" subtitle="Most recent first. Quantify everything you can."/>
      {items.map((exp, i) => (<ItemCard key={exp.id} title={`Experience #${i + 1}`} onDelete={items.length > 1 ? () => del(exp.id) : undefined}>
          ...
          <Field label="Description" hint="Bullet points. Start with action verbs. Add numbers.">
            <textarea className="input" rows={4} value={exp.description} onChange={(e) => update(exp.id, { description: e.target.value })} placeholder="• Built X that improved Y by 30%&#10;• Led migration from A to B&#10;• Owned the C pipeline..."/>
          </Field>
        </ItemCard>))}
...
// ── Education ───────────────────────────────────────────────────────
function EducationSection({ items, update, add, del }) {
    return (<>
      <FormHeader icon="🎓" title="Education"/>
      {items.map((edu, i) => (<ItemCard key={edu.id} title={`Education #${i + 1}`} onDelete={items.length > 1 ? () => del(edu.id) : undefined}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
...
// ── Skills ──────────────────────────────────────────────────────────
function SkillsSection({ skills, setSkill }) {
    return (<>
...
// ── Projects ────────────────────────────────────────────────────────
function ProjectsSection({ items, update, add, del }) {
    return (<>
      ...
      {items.map((p, i) => (<ItemCard key={p.id} title={`Project #${i + 1}`} onDelete={() => del(p.id)}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
...
// ── Certificates ────────────────────────────────────────────────────
function CertsSection({ items, update, add, del }) {
    return (<>
      ...
      {items.map((c, i) => (<ItemCard key={c.id} title={`Certificate #${i + 1}`} onDelete={() => del(c.id)}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
      ...
      {items.map((l, i) => (<ItemCard key={l.id} title={`Language #${i + 1}`} onDelete={() => del(l.id)}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <Field label="Proficiency">
              <select className="input" value={l.proficiency} onChange={(e) => update(l.id, { proficiency: e.target.value })}>
...
// ── Declaration ─────────────────────────────────────────────────────
function DeclarationSection({ data, set }) {
    return (<>
