...
function FaceEnroll({ onSuccess, onCancel }) {
    const videoRef = (0, react_1.useRef)(null);
    const streamRef = (0, react_1.useRef)(null);
    const [state, setState] = (0, react_1.useState)('idle');
    const [message, setMessage] = (0, react_1.useState)('');
    const [captured, setCaptured] = (0, react_1.useState)(0); // 0..5 frames captured
    const [faceBox, setFaceBox] = (0, react_1.useState)(null);
    const [faceLoaded, setFaceLoaded] = (0, react_1.useState)(false);
    const descriptorsRef = (0, react_1.useRef)([]);
    const captureIntervalRef = (0, react_1.useRef)(null);
    function stopCamera() {
    ...
    async function startEnrollment() {
        ...
        try {
            ...
            if (!fapi)
                throw new Error('face-api.js not loaded yet. Try again in a moment.');
    ...
    function captureFrames() {
        ...
        captureIntervalRef.current = setInterval(async () => {
            if (!videoRef.current || descriptorsRef.current.length >= 5)
                return;
    ...
    (0, react_1.useEffect)(() => () => stopCamera(), []);
    const vidW = 280, vidH = 210;
    const showVideo = state === 'ready' || state === 'capturing' || state === 'loading_model';
    return (<>
      <script_1.default src="https://cdn.jsdelivr.net/npm/@vladmandic/face-api/dist/face-api.js" strategy="lazyOnload" onLoad={() => setFaceLoaded(true)}/>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, padding: 4 }}>
        ...
        <div style={{
          ...
          <video ref={videoRef} width={vidW} height={vidH} muted playsInline style={{ display: showVideo ? 'block' : 'none', objectFit: 'cover', transform: 'scaleX(-1)' }}/>

          {/* Face box */}
          {faceBox && showVideo && (<div style={{
                position: 'absolute',
          ...
          {state === 'capturing' && (<div style={{ position: 'absolute', bottom: 8, left: 8, right: 8 }}>
          ...
          {/* States */}
          {state === 'idle' && (<div style={SC}><div style={{ fontSize: 36, opacity: 0.25 }}>👤</div></div>)}
          {state === 'loading_model' && (<div style={SC}><div style={{ fontSize: 22, animation: 'spin 1s linear infinite' }}>⬡</div></div>)}
          {state === 'processing' && (<div style={SC}><div style={{ fontSize: 22, animation: 'spin 1s linear infinite' }}>⬡</div><div style={{ fontSize: 11, color: 'var(--t3)', marginTop: 6 }}>Processing...</div></div>)}
          {state === 'success' && (<div style={{ ...SC, background: 'rgba(5,150,105,0.15)' }}><div style={{ fontSize: 36, color: 'var(--green)' }}>✓</div><div style={{ fontSize: 12, color: 'var(--green)', fontWeight: 700, marginTop: 4 }}>Enrolled!</div></div>)}
          {state === 'error' && (<div style={SC}><div style={{ fontSize: 32 }}>⚠️</div></div>)}
        </div>

        {/* Frame count dots */}
        {(state === 'capturing' || state === 'success') && (<div style={{ display: 'flex', gap: 6 }}>
        ...
        {/* Actions */}
        {state === 'idle' && (<button onClick={startEnrollment} className="btn-primary" style={{ width: '100%' }}>
            📷 Start Face Enrollment
          </button>)}
        {(state === 'ready' || state === 'capturing') && (<button onClick={() => { stopCamera(); setState('idle'); setCaptured(0); setMessage(''); }} className="btn-ghost" style={{ width: '100%' }}>
            Cancel
          </button>)}
        {state === 'error' && (<div style={{ display: 'flex', gap: 8, width: '100%' }}>
        ...
        {state === 'success' && (<button onClick={onCancel || (() => { })} className="btn-primary" style={{ width: '100%' }}>Done ✓</button>)}

        {state === 'idle' && (<div style={{ fontSize: 11, color: 'var(--t4)', textAlign: 'center', lineHeight: 1.5 }}>
