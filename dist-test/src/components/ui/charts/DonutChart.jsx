...
function DonutChart({ segments, total, centerLabel = 'Total', size = 180, showLegend = true, }) {
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
      <svg width={size} height={size} role="img" aria-label={`Donut chart, ${centerLabel}: ${computedTotal}`}>
        {/* Track */}
        <circle cx={cx} cy={cy} r={r} fill="none" stroke="var(--bg3, #f3f4f6)" strokeWidth={strokeW}/>

        {/* Segment arcs */}
        {arcs.map((arc, i) => arc.pct > 0 ? (<path key={i} d={arc.d} fill="none" stroke={arc.color} strokeWidth={strokeW} strokeLinecap="butt" style={{ transition: 'all 0.8s ease' }}/>) : null)}

        {/* Center text */}
        <text x={cx} y={cy - 8} textAnchor="middle" fontSize="26" fontWeight="800" fill="var(--t1, #1f2937)">
          {computedTotal}
        </text>
        <text x={cx} y={cy + 14} textAnchor="middle" fontSize="11" fill="var(--t3, #9ca3af)">
          {centerLabel}
        </text>
      </svg>
      ...
      {showLegend && (<div style={{
          ...
          {arcs.map((arc, i) => (<div key={i} style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: 8,
                    fontSize: 11,
