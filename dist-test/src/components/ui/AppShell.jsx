...
const PinsBadge_1 = __importDefault(require("@/components/pins/PinsBadge"));
// Lazy-load avatar to avoid SSR issues with Three.js / VRoid
const AvatarMentorWidget = (0, react_1.lazy)(() => Promise.resolve().then(() => __importStar(require('@/components/avatar/AvatarMentorWidget'))));
...
function GlobalAvatar({ user, profile }) {
    const cOS = (0, CareerOSContext_1.useCareerOS)();
    const pathname = (0, navigation_1.usePathname)();
    const { onboardingStep, setOnboardingStep, resumeGenerated, roadmapGenerated, completedQuests, javaTestPassed } = cOS;
    const [mounted, setMounted] = (0, react_1.useState)(false);
    const [showSpeechBubble, setShowSpeechBubble] = (0, react_1.useState)(true);
    ...
    return (<>
      ...
      {isCentered && !minimized && (<div style={{
      ...
      {minimized && (<button onClick={() => setMinimized(false)} style={{
                position: 'fixed',
                bottom: 24,
                right: 24,
                zIndex: 1000,
      ...
      <div style={isCentered ? {
        ...
        <div style={{ width: '100%', height: '100%', overflow: 'hidden', borderRadius: isCentered ? 20 : 18 }}>
          <react_1.Suspense fallback={<div style={{ width: '100%', height: '100%', background: 'var(--card)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--t3)' }}>
              Loading mentor...
            </div>}>
            <AvatarMentorWidget userId={user?.id} careerProfile={profile || undefined} teacherId={teacherId} minimized={minimized} setMinimized={setMinimized} showSpeechBubble={showSpeechBubble} setShowSpeechBubble={setShowSpeechBubble} onboardingStep={onboardingStep} setOnboardingStep={setOnboardingStep}/>
          </react_1.Suspense>
        </div>
      </div>
    </>);
}
const isGroup = (n) => 'children' in n;
...
function AppShell({ children }) {
    ...
    const { profile } = (0, useCareerProfile_1.useCareerProfile)();
    const { data: notifData } = (0, hooks_1.useNotifications)();
    const wsConnected = (0, useAppStore_1.useAppStore)(s => s.wsConnected);
    // Unified Career OS Context nervous system
    const cOS = (0, CareerOSContext_1.useCareerOS)();
    const { careerScore, dnaScore, trustScore, missionStreak, onboardingAnswers, vaultItems, completedMissions, jdMissingSkills, theme, focusMode, toggleTheme, toggleFocusMode, pins, } = cOS;
    ...
    (0, react_1.useEffect)(() => { setMobileOpen(false); }, [pathname]);
    (0, react_1.useEffect)(() => {
    ...
    function NavLink({ href, icon, label, badge, indent = false }) {
        const active = isPathActive(pathname, href);
        return (<link_1.default href={href} className={`nav-item${active ? ' active' : ''}`} style={indent && !collapsed ? { paddingLeft: 32 } : undefined}>
        ...
        {collapsed && badge && unread > 0 && (<span style={{ position: 'absolute', top: 5, right: 5, width: 7, height: 7, borderRadius: '50%', background: 'var(--coral)', border: '2px solid var(--bg2)' }}/>)}
      </link_1.default>);
    }
    function NavGroupHeader({ group }) {
        ...
        return (<>
        <button type="button" onClick={() => toggleGroup(group.label)} className={`nav-item${hasActiveChild ? ' active' : ''}`} style={{ width: '100%', background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
    ...
    return (<div className="app-shell">
      ...
      <aside className={`sidebar${collapsed || focusMode ? ' collapsed' : ''}${mobileOpen ? ' open' : ''}`} style={{
        ...
        <div className="sidebar-logo">
          <link_1.default href="/dashboard" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 10 }}>
            <div className="logo-mark">Pi</div>
            {!collapsed && !focusMode && (<div>
                <div className="logo-text">PinIT</div>
                <div className="logo-sub">Career OS</div>
              </div>)}
          </link_1.default>
          {!collapsed && !focusMode && (<button onClick={() => setCollapsed(true)} title="Collapse (⌘[)" style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--t4)', fontSize: 18, padding: '2px 6px', borderRadius: 6, lineHeight: 1 }}>
              ‹
            </button>)}
        </div>
          ...
          {nav.map(sec => {
            const filteredItems = sec.items.filter(item => {
                ...
                if (isStudent) {
                    ...
                    if (!unlocked.includes(item.href))
                        return false;
            ...
            return (<div key={sec.section}>
                ...
                {filteredItems.map(item => isGroup(item)
          ...
          {!collapsed && !focusMode && isStudent && (() => {
            ...
            return (<div style={{
                ...
                <div style={{ fontSize: 11, color: 'var(--t2)', marginTop: 2 }}>
                  Next: <link_1.default href={nextStepHref} style={{ color: 'var(--accent)', fontWeight: 700, textDecoration: 'none' }}>{nextStepText}</link_1.default>
                </div>
              </div>);
        })()}
        </nav>
        ...
        <div className="sidebar-footer">
          ...
          {collapsed && !focusMode && (<button onClick={() => setCollapsed(false)} className="nav-item" style={{ justifyContent: 'center', marginTop: 6 }} title="Expand (⌘[)">
              <span className="nav-icon">›</span>
            </button>)}

          {!collapsed && !focusMode && isStudent && (<link_1.default href="/pricing" style={{ textDecoration: 'none', display: 'block', marginTop: 4 }}>
              <div style={{
          ...
          {!collapsed && !focusMode && !cOS.demoTabsUnlocked && (<button onClick={() => {
                cOS.setDemoTabsUnlocked(true);
                useAppStore_1.toast.success('Demo Bypass Activated 🔓', 'All modules unlocked for evaluation.');
            }} style={{
                width: '100%',
                marginTop: 6,
                padding: '7px 10px',
          ...
          {!collapsed && !focusMode && (<div style={{
              ...
              <button onClick={() => logout().then(() => router.push('/login'))} title="Logout" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--t4)', fontSize: 14, padding: 4, borderRadius: 6, flexShrink: 0 }}>
                ⏻
              </button>
            </div>)}
        </div>
      ...
      <div className="main-area">
        ...
        <header className="topbar">
          {/* Mobile burger */}
          <button onClick={() => setMobileOpen(o => !o)} className="mobile-menu-btn" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--t2)', fontSize: 18, padding: 4, borderRadius: 6, display: 'none' }}>
            ☰
          </button>

          {/* If focusMode is active, allow returning with a floating action bar brand */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {focusMode && (<div onClick={toggleFocusMode} title="Exit Focus Mode" style={{
          ...
          {isStudent && !focusMode && (<div className="topbar-scores" style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
              {[
                { icon: 'ATS', val: careerScore, color: 'var(--teal)' },
                { icon: 'DNA', val: dnaScore, color: 'var(--purple)' },
                { icon: '🛡', val: trustScore, color: 'var(--green)' },
            ].map(p => (<div key={p.icon} style={{
              ...
              <link_1.default href="/vault" title="Vault Secure Area" style={{
              ...
              {/* Pin Balance */}
              <PinsBadge_1.default size="sm" showLink/>
            </div>)}

          {/* Theme Switcher & Focus Mode Toggles Control Group */}
          <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginLeft: 8 }}>
            {/* Theme Toggle Button */}
            <button onClick={toggleTheme} title={theme === 'light' ? 'Switch to Dark Mode' : 'Switch to Light Mode'} style={{
            ...
            {isStudent && (<button onClick={toggleFocusMode} title={focusMode ? 'Deactivate Focus Mode' : 'Activate Focus Mode'} style={{
          ...
          {/* Bell */}
          <link_1.default href="/notifications" style={{ position: 'relative', color: 'var(--t2)', textDecoration: 'none', padding: 6, borderRadius: 8, display: 'flex', alignItems: 'center' }}>
            🔔
            {unread > 0 && (<span style={{
