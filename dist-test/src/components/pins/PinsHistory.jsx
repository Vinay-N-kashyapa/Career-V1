...
function PinsHistory({ limit = 10 }) {
    const { pinHistory } = (0, CareerOSContext_1.useCareerOS)();
    const shown = pinHistory.slice(0, limit);
    if (!shown.length) {
        return (<div style={{ textAlign: 'center', padding: '24px 16px', color: 'var(--t3)', fontSize: 13 }}>
        No pin transactions yet. Complete missions to start earning!
      </div>);
    }
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      {shown.map(tx => (<div key={tx.id} style={{
