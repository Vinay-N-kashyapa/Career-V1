...
function PinsGate({ featureKey, onUnlocked, children, mode = 'wrap', buttonLabel, buttonClass, disabled }) {
    const { pins, spendPins, canAfford } = (0, CareerOSContext_1.useCareerOS)();
    const [showConfirm, setShowConfirm] = (0, react_1.useState)(false);
    const meta = CareerOSContext_1.PIN_COSTS[featureKey];
    if (!meta) {
    ...
    return (<>
      ...
      {showConfirm && (<div style={{
          ...
          <div style={{
            ...
            {affordable ? (<div style={{ display: 'flex', gap: 10 }}>
              ...
              </div>) : (<div style={{ display: 'flex', gap: 10, flexDirection: 'column' }}>
                ...
                ].map((tip, i) => (<div key={i} style={{ display: 'flex', gap: 8, fontSize: 12, color: 'var(--t2)' }}>
                    <span>{tip.icon}</span><span>{tip.text}</span>
                  </div>))}
                <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
                  <link_1.default href="/pricing" style={{ textDecoration: 'none', flex: 1 }}>
                    <button className="btn-primary" style={{ width: '100%', justifyContent: 'center' }}>💳 Buy Pins</button>
                  </link_1.default>
                  <link_1.default href="/missions" style={{ textDecoration: 'none', flex: 1 }}>
