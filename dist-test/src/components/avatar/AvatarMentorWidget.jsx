...
exports.default = AvatarMentorWidget;
const react_1 = require("react");
const usePersonalAvatarMemory_1 = require("./hooks/usePersonalAvatarMemory");
const useFacialEmotionDetection_1 = require("./hooks/useFacialEmotionDetection");
...
class AvatarScene {
                    ...
                    if (this.faceMesh && this.faceMesh.morphTargetDictionary) {
                        ...
                        vowels.forEach(v => {
                            ...
                            if (foundKey) {
                                ...
                                console.log(`VRoidMentorWidget: Mapped vowel ${v} to morph target index: ${dict[foundKey]} (key: "${foundKey}")`);
                            }
                            else {
                                console.warn(`VRoidMentorWidget: Could not map morph target for vowel ${v}`);
                            }
                        });
                    }
                    else {
                        console.warn("VRoidMentorWidget: No face mesh with morph target dictionary found!");
                    }
                    resolve();
                }, undefined, () => {
    ...
    setState(s) { if (this.animState === s)
        return; this.animState = s; this.animT = 0; }
    loop() {
        if (this.disposed)
            return;
...
function AvatarMentorWidget({ userId = 'guest', careerProfile, teacherId = 'priya', minimized, setMinimized, showSpeechBubble, setShowSpeechBubble, onboardingStep, setOnboardingStep, activeQuest, }) {
    const [input, setInput] = (0, react_1.useState)('');
    const [messages, setMessages] = (0, react_1.useState)([]);
    const [loading, setLoading] = (0, react_1.useState)(false);
    const [speaking, setSpeaking] = (0, react_1.useState)(false);
    const [localMinimized, setLocalMinimized] = (0, react_1.useState)(false);
    const [mlRecs, setMlRecs] = (0, react_1.useState)([]);
    ...
    const [subtitleRole, setSubtitleRole] = (0, react_1.useState)(null);
    ...
    (0, react_1.useEffect)(() => {
        ...
        const ro = new ResizeObserver(e => {
            const el = e[0];
            if (el)
                scene.resize(el.contentRect.width, el.contentRect.height);
        });
    ...
    const memory = (0, usePersonalAvatarMemory_1.usePersonalAvatarMemory)(userId);
    const emotionAI = (0, useFacialEmotionDetection_1.useFacialEmotionDetection)();
    ...
    (0, react_1.useEffect)(() => {
        ...
        fetch('/api/avatar/context', { credentials: 'include' })
            ...
            if (avatarMemory?.conversationHistory?.length)
                memory.importMemory(avatarMemory);
            if (mlRecommendations?.length)
                setMlRecs(mlRecommendations);
        })
            .catch(() => { });
    }, [userId]);
    // Sync career profile to avatar memory
    (0, react_1.useEffect)(() => {
        if (!careerProfile || userId === 'guest')
            return;
    ...
    (0, react_1.useEffect)(() => { bottomRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [messages, loading]);
    ...
    const sendMessage = (0, react_1.useCallback)(async (text) => {
        ...
        const historyForAPI = memory.getConversationContext(10)
            ...
            : [{ role: c.role, content: c.content }]).flat().filter((m) => m.content);
        try {
    ...
    async function speakReply(text) {
        ...
        (0, tts_1.speakWithAvatar)(text, teacherId, () => setSpeaking(true), () => setSpeaking(false));
    }
    // Keep track of latest speaking/loading states in refs to avoid stale closures in SpeechRecognition handlers
    const speakingRef = (0, react_1.useRef)(speaking);
        ...
        const startListening = () => {
            ...
            try {
                ...
                recognition.onresult = (e) => {
                    ...
                    const matchesWakeWord = text.includes('priya') ||
    ...
    if (isMinimized) {
        return (<button onClick={() => setIsMinimized(false)} style={{
    ...
    return (<div className="avatar-mentor-widget" style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg2)' }}>
      {/* Header */}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg2)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 14px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          {messages.map((m, i) => {
            ...
            return (<div key={i} style={{
                ...
                {!isUser && (<span style={{ fontSize: 20, marginTop: 4, flexShrink: 0 }}>
                    {teacher.emoji}
                  </span>)}
                <div style={{
                    padding: '10px 14px',
          ...
          {loading && (<div style={{ display: 'flex', gap: 8, alignSelf: 'flex-start', alignItems: 'center', color: 'var(--t3)', fontSize: 12, paddingLeft: 28 }}>
        ...
        <div style={{
          ...
          {quickPrompts.map((promptText, i) => (<button key={i} type="button" onClick={() => {
        ...
        <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} style={{
          ...
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder={`Ask ${teacher.name} a question...`} style={{
            flex: 1,
            padding: '9px 14px',
            borderRadius: 20,
            border: '1px solid var(--border)',
