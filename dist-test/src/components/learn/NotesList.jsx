...
function NotesList({ selected, onSelect }) {
    ...
    return (<div style={{ marginBottom: 20 }}>
      ...
      {/* Upload zone */}
      <div onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={e => { e.preventDefault(); setDragOver(false); const f = e.dataTransfer.files[0]; if (f)
        uploadFile(f); }} onClick={() => document.getElementById('note-upload')?.click()} style={{
        ...
        <input id="note-upload" type="file" accept=".pdf,.docx,.doc,.txt" style={{ display: 'none' }} onChange={e => e.target.files?.[0] && uploadFile(e.target.files[0])}/>
      </div>

      {/* Notes list */}
      {loading ? (<div style={{ color: 'var(--t3)', fontSize: 12 }}>Loading notes...</div>) : notes.length === 0 ? (<div style={{ color: 'var(--t3)', fontSize: 12 }}>No notes yet. Upload a PDF or DOCX to start.</div>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 200, overflowY: 'auto' }}>
          {notes.map(n => (<div key={n.id} onClick={() => toggle(n.id)} style={{
