...
function ChatInterface({ sessionId, teacherId, mode, noteIds, careerContext, userId }) {
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 180px)', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
      {/* Header */}
      <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        ...
        {careerContext && (<div style={{ marginLeft: 'auto', fontSize: 10, color: 'var(--t3)', fontFamily: 'var(--font-mono)' }}>
            ATS: {careerContext.ats_score || 0}
          </div>)}
      </div>

      {/* Messages */}
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        {messages.map((m, i) => (<div key={i} style={{ display: 'flex', gap: 8, justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
            {m.role === 'assistant' && (<span style={{ fontSize: 18, flexShrink: 0, marginTop: 2 }}>{TEACHER_EMOJIS[teacherId] || '👩‍🏫'}</span>)}
            <div style={{
        ...
        {loading && (<div style={{ display: 'flex', gap: 8 }}>
            ...
            <div style={{ padding: '10px 14px', background: 'var(--bg3)', borderRadius: 10, display: 'flex', gap: 4, alignItems: 'center' }}>
              {[0, 1, 2].map(i => (<span key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--t3)', display: 'inline-block', animation: `pulse 1.2s ${i * 0.2}s infinite` }}/>))}
            </div>
          </div>)}
        <div ref={bottomRef}/>
      </div>
      ...
      <div style={{ padding: '12px 16px', borderTop: '1px solid var(--border)', display: 'flex', gap: 8, flexShrink: 0 }}>
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && send()} placeholder={`Ask ${TEACHER_NAMES[teacherId] || 'teacher'}...`} disabled={loading} style={{
            flex: 1, background: 'var(--bg3)', border: '1px solid var(--border)',
            borderRadius: 8, padding: '9px 14px', color: 'var(--t1)',
            fontSize: 13, fontFamily: 'var(--font-body)', outline: 'none',
        }}/>
        <button onClick={() => send()} disabled={loading || !input.trim()} className="btn-primary" style={{ flexShrink: 0 }}>
          →
        </button>
      </div>
