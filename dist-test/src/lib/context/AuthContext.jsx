...
function sbUserToAppUser(sbUser, profile) {
    ...
    return {
        ...
        email: sbUser.email || '',
        displayName: profile?.displayName || sbUser.user_metadata?.display_name || 'User',
        role: role,
        registerNumber: profile?.registerNumber,
        selectedTeacherId: profile?.selectedTeacherId,
        atsScore: profile?.ats_score,
        trustScore: profile?.trust_score,
        careerDnaScore: profile?.career_dna_score,
        missionStreak: profile?.mission_streak,
    };
}
function AuthProvider({ children }) {
    const [user, setUser] = (0, react_1.useState)(null);
    const [loading, setLoading] = (0, react_1.useState)(true);
    const loadProfile = (0, react_1.useCallback)(async (sbUser) => {
        try {
            let profile = await (0, supabaseService_1.getUserProfile)(sbUser.id);
            if (!profile) {
                ...
                profile = {
                    ...(isDemoEmail(sbUser.email || '') ? supabaseService_1.DEMO_PROFILE : supabaseService_1.EMPTY_PROFILE),
                    uid: sbUser.id,
                    email: sbUser.email || '',
                ...
                await (0, supabaseService_1.createUserProfile)(sbUser.id, profile);
            }
            else {
                // Self-healing check for existing profiles of default accounts
                const emailLower = sbUser.email?.toLowerCase();
                let expectedRole = null;
                if (emailLower === 'admin@pinit.in' && profile.role !== 'admin')
                    expectedRole = 'admin';
                else if (emailLower === 'rec@pinit.in' && profile.role !== 'recruiter')
                    expectedRole = 'recruiter';
                else if (emailLower === 'con@pinit.in' && profile.role !== 'consultant')
                    expectedRole = 'consultant';
                if (expectedRole) {
                    profile.role = expectedRole;
                    await (0, supabaseService_1.updateUserProfile)(sbUser.id, { role: expectedRole });
                }
            }
            await (0, supabaseService_1.ensureSeedData)(sbUser.id, profile);
    ...
    (0, react_1.useEffect)(() => {
        ...
        supabaseClient_1.supabase.auth.getSession().then(({ data: { session } }) => {
            ...
            if (sbUser) {
                ...
                const channel = supabaseClient_1.supabase
                    ...
                    const profile = (0, supabaseService_1.mapRowToProfile)(payload.new);
                    const emailLower = sbUser.email?.toLowerCase();
                    let role = profile?.role || 'student';
                    if (emailLower === 'admin@pinit.in')
                        role = 'admin';
        ...
        const { data: { subscription } } = supabaseClient_1.supabase.auth.onAuthStateChange(async (event, session) => {
            const sbUser = session?.user ?? null;
            if (sbUser) {
        ...
        try {
            ...
            if (error) {
                ...
                if (isDefaultUser) {
                    ...
                    const profile = {
                        ...supabaseService_1.DEMO_PROFILE,
                        uid: sbUser.id,
                        email,
            ...
            await (0, supabaseService_1.updateUserProfile)(sbUser.id, { _passwordSecret: password });
            let profile = await (0, supabaseService_1.getUserProfile)(sbUser.id);
            if (!profile) {
                let role = 'student';
                let displayName = 'User';
            ...
            if (isDefaultUser) {
                ...
                const profile = {
                    ...(isDemoEmail(email) ? supabaseService_1.DEMO_PROFILE : supabaseService_1.EMPTY_PROFILE),
                    uid: mockSbUser.id,
                    email,
                    username: email.split('@')[0],
                    displayName,
    ...
    const signup = async (data) => {
        ...
        try {
            const { data: resData, error } = await supabaseClient_1.supabase.auth.signUp({
                email,
            ...
            if (!sbUser)
                throw new Error('SignUp succeeded but returned no user.');
            ...
            if (!resData.session) {
                ...
                const { error: signInError } = await supabaseClient_1.supabase.auth.signInWithPassword({
                    email,
                    password: data.password,
                });
                if (signInError) {
    ...
    return (<Ctx.Provider value={{ user, loading, login, signup, logout, refresh }}>
      {children}
    </Ctx.Provider>);
}
function useAuth() {
