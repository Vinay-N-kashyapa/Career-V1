...
exports.PIN_EARN = exports.PIN_COSTS = void 0;
exports.CareerOSProvider = CareerOSProvider;
exports.useCareerOS = useCareerOS;
...
const CareerOSContext = (0, react_1.createContext)(null);
// ─── Defaults ────────────────────────────────────────────────────────────────
const DEFAULT_VAULT_ITEMS = []; // Empty vault by default for new redone setup
// ─── Provider ─────────────────────────────────────────────────────────────────
function CareerOSProvider({ children }) {
    const { user } = (0, AuthContext_1.useAuth)();
    const userId = user?.id || 'guest';
    const [vaultItems, setVaultItems] = (0, react_1.useState)([]);
    const [onboardingAnswers, setOnboardingAnswers] = (0, react_1.useState)({ role: '', education: '', skills: '', experience: '', hasCompleted: false });
    const [completedMissions, setCompletedMissions] = (0, react_1.useState)([]);
    const [jdMissingSkills, setJdMissingSkillsState] = (0, react_1.useState)([]);
    const [xp, setXp] = (0, react_1.useState)(120);
    const [missionStreak, setMissionStreak] = (0, react_1.useState)(0); // Start streak at 0
    const [theme, setTheme] = (0, react_1.useState)('light');
    const [focusMode, setFocusMode] = (0, react_1.useState)(false);
    const [isLoaded, setIsLoaded] = (0, react_1.useState)(false);
    // ── Progression State ──
    const [onboardingStep, setOnboardingStepState] = (0, react_1.useState)(0);
    const [resumeGenerated, setResumeGeneratedState] = (0, react_1.useState)(false);
    const [roadmapGenerated, setRoadmapGeneratedState] = (0, react_1.useState)(false);
    const [completedQuests, setCompletedQuestsState] = (0, react_1.useState)([]);
    const [javaTestPassed, setJavaTestPassedState] = (0, react_1.useState)(false);
    const [recruiterVisible, setRecruiterVisibleState] = (0, react_1.useState)(false);
    const [unlockedTabs, setUnlockedTabs] = (0, react_1.useState)(['/dashboard', '/resume', '/career-builder', '/quests']);
    const [forceShowCareerBuilder, setForceShowCareerBuilderState] = (0, react_1.useState)(false);
    const [demoTabsUnlocked, setDemoTabsUnlockedState] = (0, react_1.useState)(false);
    // ── Pin state ──────────────────────────────────────────────────────────
    const [pins, setPins] = (0, react_1.useState)(100);
    const [pinHistory, setPinsHistory] = (0, react_1.useState)([]);
    ...
    const save = (0, react_1.useCallback)((key, data) => {
    ...
    // ─── Pin helpers ───────────────────────────────────────────────────────
    const pushTransaction = (0, react_1.useCallback)((tx, newHistory) => {
        const trimmed = newHistory.slice(0, 100); // keep last 100
        setPinsHistory(trimmed);
        save(keys.pinHist, trimmed);
    }, [keys.pinHist, save]);
    const earnPins = (0, react_1.useCallback)((source, overrideAmount, reason) => {
        const amount = overrideAmount ?? exports.PIN_EARN[source] ?? 0;
        if (amount <= 0)
            return;
        // Update Firestore in background
        client_1.api.post('/api/pins/earn', { source, amount }).catch(() => { });
        setPins(prev => {
            const next = prev + amount;
            save(keys.pins, next);
            return next;
        });
        const tx = { id: `tx_${Date.now()}`, type: 'earn', amount, reason: reason ?? source.replace(/_/g, ' '), source, timestamp: Date.now() };
        setPinsHistory(prev => {
            const updated = [tx, ...prev].slice(0, 100);
            save(keys.pinHist, updated);
            return updated;
        });
        useAppStore_1.toast.success(`+${amount} Pins Earned ⚡`, reason ?? source.replace(/_/g, ' '));
    }, [keys.pins, keys.pinHist, save]);
    const canAfford = (0, react_1.useCallback)((featureKey) => {
        const cost = exports.PIN_COSTS[featureKey]?.cost ?? 0;
        return pins >= cost;
    }, [pins]);
    const spendPins = (0, react_1.useCallback)((featureKey, customReason) => {
        const meta = exports.PIN_COSTS[featureKey];
        if (!meta)
            return true; // no cost defined = free
        if (pins < meta.cost) {
            useAppStore_1.toast.error(`Insufficient Pins 📌`, `Need ${meta.cost} pins for ${meta.label}. Earn more by completing missions, exams, or sessions.`);
            return false;
        }
        // Update Firestore in background
        client_1.api.post('/api/pins/spend', { featureKey, cost: meta.cost }).catch(() => { });
        setPins(prev => {
            const next = prev - meta.cost;
            save(keys.pins, next);
            return next;
        });
        const tx = { id: `tx_${Date.now()}`, type: 'spend', amount: meta.cost, reason: customReason ?? meta.label, source: featureKey, timestamp: Date.now() };
        setPinsHistory(prev => {
            const updated = [tx, ...prev].slice(0, 100);
            save(keys.pinHist, updated);
            return updated;
    ...
    const addPurchasedPins = (0, react_1.useCallback)((amount, packName) => {
        // Update Firestore in background
        client_1.api.post('/api/pins/purchase', { amount, packName }).catch(() => { });
        setPins(prev => {
            const next = prev + amount;
            save(keys.pins, next);
            return next;
        });
        const tx = { id: `tx_${Date.now()}`, type: 'earn', amount, reason: `Purchased: ${packName}`, source: 'purchase', timestamp: Date.now() };
        setPinsHistory(prev => {
            const updated = [tx, ...prev].slice(0, 100);
            save(keys.pinHist, updated);
            return updated;
        });
        useAppStore_1.toast.success(`⚡ ${amount} Pins Added!`, `${packName} pack activated successfully.`);
    }, [keys.pins, keys.pinHist, save]);
    // ─── Daily Pins Grant Scheduler ───────────────────────────────────────────
    (0, react_1.useEffect)(() => {
        if (!isLoaded || userId === 'guest')
            return;
        ...
        if (!lastGrantStr || now - parseInt(lastGrantStr) >= oneDay) {
            ...
            // Update Firestore in background
            client_1.api.post('/api/pins/earn', { source: 'daily_login', amount: dailyAmount }).catch(() => { });
            // Log transaction
            const tx = {
                id: `tx_daily_${now}`,
                type: 'earn',
            ...
            useAppStore_1.toast.success(`⚡ Daily Pins Issued!`, `+${dailyAmount} Pins added for your daily login.`);
        }
    }, [isLoaded, userId, user?.role, keys.pins, keys.pinHist]);
    // ─── Progression Setters ───────────────────────────────────────────────────
    const setOnboardingStep = (0, react_1.useCallback)((step) => {
        setOnboardingStepState(step);
        save(keys.obStep, step);
    }, [keys.obStep, save]);
    const setResumeGenerated = (0, react_1.useCallback)((val) => {
    ...
    const setRoadmapGenerated = (0, react_1.useCallback)((val) => {
    ...
    const generateFusedRoadmap = (0, react_1.useCallback)(async (skillTags, weakAreas) => {
        ...
        try {
            ...
            if (res && res.ok && Array.isArray(res.modules) && res.modules.length > 0) {
                ...
                useAppStore_1.toast.success('Quest Roadmap Active! 🗺️', 'Fused onboarding preferences and resume skills to create customized quests.');
                return normalized;
            }
        }
        catch (err) {
    ...
    const addCompletedQuest = (0, react_1.useCallback)((questId) => {
    ...
    const setJavaTestPassed = (0, react_1.useCallback)((val) => {
    ...
    const setRecruiterVisible = (0, react_1.useCallback)((val) => {
        setRecruiterVisibleState(val);
        save(keys.recVis, val);
    }, [keys.recVis, save]);
    const setForceShowCareerBuilder = (0, react_1.useCallback)((val) => {
        setForceShowCareerBuilderState(val);
        save(keys.forceShowCareer, val);
    }, [keys.forceShowCareer, save]);
    const setDemoTabsUnlocked = (0, react_1.useCallback)((val) => {
    ...
    const toggleTheme = () => setTheme(p => { const n = p === 'light' ? 'dark' : 'light'; useAppStore_1.toast.info('Theme Switched', `${n === 'light' ? 'Light ☀️' : 'Dark 🌙'} Mode`); return n; });
    const toggleFocusMode = () => setFocusMode(p => { const n = !p; useAppStore_1.toast[n ? 'success' : 'info'](n ? 'Focus Mode On 🤫' : 'Focus Mode Off', n ? 'Distractions hidden.' : 'Standard layout restored.'); return n; });
    const addVaultItem = async (item) => {
        ...
        useAppStore_1.toast.success('🔒 Added to Vault', `"${item.title}" saved securely.`);
        if (userId !== 'guest') {
            try {
    ...
    const updateVaultItem = (id, updates) => {
        const updated = vaultItems.map(item => {
            ...
            if (updates.verified && !item.verified) {
                useAppStore_1.toast.success('✓ Proof Verified', `"${item.title}" is officially verified!`);
    ...
    const completeMission = (missionId) => {
        ...
        earnPins('mission_complete');
        if (newStreak % 7 === 0)
            earnPins('streak_bonus', undefined, `${newStreak}-day streak bonus!`);
        useAppStore_1.toast.success('🔥 Mission Done!', `Streak: ${newStreak} days · +10 Pins earned`);
    };
    const setOnboarding = (answers) => {
        const data = { ...answers, hasCompleted: true };
        setOnboardingAnswers(data);
        save(keys.onboard, data);
        addXp(50, 'Onboarding Complete');
        earnPins('onboarding_complete');
        useAppStore_1.toast.success('🧬 Career Profile Set', `Target role: ${answers.role}`);
    };
    const setJdMissingSkills = (skills) => { setJdMissingSkillsState(skills); save(keys.gaps, skills); };
    ...
    return (<CareerOSContext.Provider value={{
...
function useCareerOS() {
    const ctx = (0, react_1.useContext)(CareerOSContext);
    if (!ctx)
