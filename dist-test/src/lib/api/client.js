...
exports.api = exports.PaywallError = exports.ApiError = void 0;
// API client — routes all /api/* calls to Supabase + Claude AI (Document 8 version)
const supabaseClient_1 = require("@/lib/supabaseClient");
const fs = __importStar(require("@/lib/supabaseService"));
const interviewAI_1 = require("@/lib/api/interviewAI");
...
class PaywallError extends ApiError {
    constructor(feature, requiredTier = 'Pro') {
        super(402, 'TIER_LIMIT_REACHED', `${feature} requires ${requiredTier}`);
...
async function firestoreRouter(method, path, body) {
    const uid = await getUid();
    const [cleanPath, queryString] = path.split('?');
    const params = new URLSearchParams(queryString || '');
    if (cleanPath === '/api/auth/me') {
        const p = await fs.getUserProfile(uid);
        return { user: { id: uid, ...p } };
    }
    if (cleanPath === '/api/auth/logout')
        return { ok: true };
    if (cleanPath === '/api/auth/profile') {
        await fs.updateUserProfile(uid, body);
        const p = await fs.getUserProfile(uid);
        return { ok: true, user: { id: uid, ...p } };
    }
    if (cleanPath === '/api/auth/teacher') {
        const { teacherId } = body;
        await fs.updateUserProfile(uid, { selectedTeacherId: teacherId });
        return { ok: true };
    }
    ...
    if (cleanPath.startsWith('/api/auth/'))
        return { ok: true };
    if (cleanPath === '/api/missions/today') {
        const m = await fs.getTodayMissions(uid);
        return { missions: m };
    }
    if (cleanPath === '/api/missions/history') {
        const m = await fs.getMissionHistory(uid);
        return { missions: m };
    }
    if (cleanPath === '/api/missions/submit') {
        const { missionId, ...rest } = body;
        await fs.submitMission(uid, missionId, rest);
        return { ok: true };
    }
    if (cleanPath === '/api/missions/generate-custom-skill' && method === 'POST') {
        const { targetRole, skill } = body;
        await fs.generateCustomSkillQuests(uid, targetRole, skill);
        return { ok: true };
    }
    if (cleanPath.startsWith('/api/missions/streak')) {
        const p = await fs.getUserProfile(uid);
        return { streak: p?.mission_streak || 0, xpTotal: p?.xp_total || 0, xpLevel: p?.xp_level || 1 };
    }
    if (cleanPath === '/api/career-dna/scores') {
        const p = await fs.getUserProfile(uid);
        return { scores: p };
    }
    if (cleanPath === '/api/career-dna/profile') {
        const p = await fs.getUserProfile(uid);
        return { profile: p };
    }
    if (cleanPath === '/api/career-dna/archetype' && method === 'PATCH') {
        const { archetype } = body;
        await fs.updateUserProfile(uid, { career_dna_archetype: archetype });
        return { ok: true, archetype };
    }
    if (cleanPath === '/api/career-dna/calculate' || cleanPath === '/api/career-dna/recalculate') {
        const p = await fs.recalculateCareerDna(uid);
        return { scores: p };
    }
    if (cleanPath.startsWith('/api/career-dna/history')) {
        const p = await fs.getUserProfile(uid);
        const months = 6;
        const now = new Date();
        const history = Array.from({ length: months }, (_, i) => { const d = new Date(now); d.setMonth(d.getMonth() - (months - 1 - i)); const pr = i / Math.max(months - 1, 1); return { date: d.toISOString().slice(0, 10), ats_score: Math.round((p?.ats_score || 45) * (0.5 + 0.5 * pr)), trust_score: Math.round((p?.trust_score || 40) * (0.5 + 0.5 * pr)), career_dna_score: Math.round((p?.career_dna_score || 35) * (0.5 + 0.5 * pr)), mission_streak: Math.round((p?.mission_streak || 0) * pr) }; });
        return { history, months };
    }
    if (cleanPath === '/api/career-twin/results')
        return { simulation: DEMO_SIMULATION };
    if (cleanPath === '/api/career-twin/run' || cleanPath === '/api/career-twin/simulate')
        return { ok: true, simulation: DEMO_SIMULATION };
    if (cleanPath.startsWith('/api/career-twin'))
        return { simulation: DEMO_SIMULATION };
    if (cleanPath === '/api/resume/structured/me') {
        const p = await fs.getUserProfile(uid);
        const sd = p?.structured_resume || null;
        return { data: sd, resumeId: sd ? `mock-resume-${uid}` : null };
    }
    ...
    if (cleanPath === '/api/career-builder/generate' && method === 'POST') {
        const { targetRole, skillTags, weakAreas, experienceLevel } = body;
    ...
    if (cleanPath === '/api/resume/generate-from-vault' && method === 'POST') {
        ...
        if (!selectedItem) {
            if (itemId === 'demo-resume-draft') {
                selectedItem = { id: 'demo-resume-draft', title: 'college_resume_draft.pdf', item_type: 'resume', description: 'Draft resume detailing Java programming, basic algorithms, database systems, and web projects.' };
            }
            else if (itemId === 'demo-cert') {
                selectedItem = { id: 'demo-cert', title: 'java_basic_cert.pdf', item_type: 'certification', description: 'Certified in Java basic syntax, variables, loop control, classes, and OOP.' };
            }
            else {
                selectedItem = { id: itemId, title: 'Vault Document', item_type: 'resume', description: 'Java SDE Candidate resume with projects in React and Node.js.' };
            }
        }
...
Ensure the JSON output is strictly valid and contains no extra text or markdown formatting. Just return the JSON.`;
        ...
        return {
            ...
            analysis: {
                ...
                format_quality: 85,
                skill_tags: parsed.skills?.technical?.split(',').map((s) => s.trim()) || [],
    ...
    if (cleanPath.includes('/pdf'))
        throw new ApiError(503, 'PDF_SERVICE_UNAVAILABLE', 'PDF export requires the backend server.');
    if (cleanPath.startsWith('/api/resume'))
        return { resumes: [], resume: null };
    if (cleanPath === '/api/vault/items' && method === 'GET') {
        const items = await fs.getVaultItems(uid);
        return { items: items || [] };
    }
    if (cleanPath === '/api/vault/stats')
        return { byType: [], summary: { total: 0, verified_total: 0 } };
    if (cleanPath === '/api/vault' && method === 'GET') {
        const items = await fs.getVaultItems(uid);
        return { items, stats: { total: (items || []).length } };
    }
    if (cleanPath === '/api/vault' && method === 'POST') {
        const id = await fs.addVaultItem(uid, body);
        return { ok: true, itemId: id, id };
    }
    if (cleanPath.startsWith('/api/vault/upload'))
        return { ok: true, itemId: `vault-${Date.now()}` };
    if (cleanPath.startsWith('/api/vault'))
        return { ok: true };
    if (cleanPath === '/api/notifications') {
        const n = await fs.getNotifications(uid);
        return { notifications: n };
    }
    if (cleanPath === '/api/notifications/mark-all-read') {
        await fs.markAllNotificationsRead(uid);
        return { ok: true };
    }
    ...
    if (cleanPath === '/api/opportunities/apply') {
        const { opportunityId } = body;
        await fs.applyToOpportunity(uid, opportunityId);
        return { ok: true };
    }
    if (cleanPath === '/api/opportunities/match')
        return { match: { match_score: 78, matched_skills: ['React', 'Node.js', 'TypeScript'], missing_skills: ['Docker', 'System Design'], source: 'local' } };
    if (cleanPath === '/api/opportunities/applications')
        return { applications: [] };
    if (cleanPath.startsWith('/api/opportunities'))
        return { opportunities: [] };
    if (cleanPath === '/api/analytics/dashboard') {
        const p = await fs.getUserProfile(uid);
        const ad = await fs.getDashboardAnalytics(uid).catch(() => null);
        const intel = Math.round((p?.career_dna_score || 0) * 0.30 + (p?.trust_score || 0) * 0.25 + (p?.ats_score || 0) * 0.25 + (p?.recruiter_visibility || 0) * 0.20);
        const cr = Math.round((p?.ats_score || 0) * 0.35 + (p?.trust_score || 0) * 0.30 + (p?.career_dna_score || 0) * 0.20 + (p?.recruiter_visibility || 0) * 0.15);
        return { scores: { ...p, career_readiness: cr }, career_readiness: cr, intelligence_score: intel, missions: ad || {}, score_history: ad?.score_history || [] };
    }
    if (cleanPath === '/api/analytics/leaderboard/preview')
        return { leaders: [{ userId: 's-5', displayName: 'Deepa Krishnan', score: 91, streak: 18 }, { userId: 's-3', displayName: 'Sneha Iyer', score: 85, streak: 12 }, { userId: uid, displayName: 'You', score: 72, streak: 7 }], userRank: 3, total: 6 };
    if (cleanPath.startsWith('/api/analytics/leaderboard')) {
        const metric = params.get('metric') || 'trust';
        return { leaders: [{ userId: 's-5', displayName: 'Deepa Krishnan', score: 91, streak: 18 }, { userId: uid, displayName: 'You', score: 72, streak: 7 }], userRank: 3, metric };
    }
    if (cleanPath.startsWith('/api/analytics')) {
        const p = await fs.getUserProfile(uid);
        return { profile: p, stats: p };
    }
    // ── Pins API ──────────────────────────────────────────────────────────────
    if (cleanPath === '/api/pins/balance') {
        const p = await fs.getUserProfile(uid);
        return { pins: p?.pins || 100, transactions: [] };
    }
    if (cleanPath === '/api/pins/earn' && method === 'POST') {
        const { source, amount } = body;
        const p = await fs.getUserProfile(uid);
        const current = p?.pins || 100;
        const newBal = current + (amount || 0);
    ...
    if (cleanPath === '/api/pins/spend' && method === 'POST') {
        const { featureKey, cost } = body;
        const p = await fs.getUserProfile(uid);
        const current = p?.pins || 100;
        if (current < (cost || 0))
            throw new ApiError(402, 'INSUFFICIENT_PINS', `Need ${cost} pins, have ${current}`);
        const newBal = current - (cost || 0);
        await fs.updateUserProfile(uid, { pins: newBal });
        return { ok: true, pins: newBal, spent: cost };
    }
    if (cleanPath === '/api/pins/purchase' && method === 'POST') {
        const { amount, packName } = body;
        const p = await fs.getUserProfile(uid);
        const current = p?.pins || 100;
        const newBal = current + (amount || 0);
        await fs.updateUserProfile(uid, { pins: newBal });
    ...
    if (cleanPath === '/api/payment/status') {
        ...
        return { tier, endsAt: tier !== 'free' ? new Date(Date.now() + 30 * 86400000).toISOString() : null, planName: tier === 'pro' ? 'Pro' : 'Free', limits: { aiInterviews: 3, resumeUploads: 2 } };
    }
    if (cleanPath === '/api/payment/plans')
        return { plans: [{ id: 'free', name: 'Free', price: 0, features: ['3 AI interviews/month', '2 resume uploads', 'Full Career DNA'] }, { id: 'pro', name: 'Pro', price: 49900, features: ['Unlimited everything', 'AI Resume Improve'] }] };
    if (cleanPath === '/api/payment/create-order') {
        const { planId } = body;
        return { orderId: `dev_order_${Date.now()}`, amount: planId === 'pro' ? 49900 : 4999900, keyId: 'rzp_test_dev', devMode: true };
    }
    if (cleanPath === '/api/payment/verify') {
        await fs.updateUserProfile(uid, { subscription_tier: 'pro' });
        return { ok: true, tier: 'pro', message: 'Pro plan activated!' };
    }
    if (cleanPath.startsWith('/api/payment'))
        return { ok: true };
    // ── Interview with real Claude AI ────────────────────────────────────────
    if (cleanPath === '/api/interview/start' && method === 'POST') {
        const { mode = 'hr', domain, pressureMode = 'normal', persona } = body;
        const opening = await (0, interviewAI_1.aiInterviewStart)({ mode, domain, pressureMode, persona });
    ...
    if (cleanPath === '/api/interview/respond' && method === 'POST') {
        const { sessionId, response, mode = 'hr', pressureMode = 'normal' } = body;
        let transcript = _transcripts.get(sessionId);
        if (!transcript) {
            const s = await fs.getInterviewSession(uid, sessionId);
            transcript = (s?.transcript || []).map((t) => ({ role: t.role, content: t.content }));
            _transcripts.set(sessionId, transcript);
        }
        const reply = await (0, interviewAI_1.aiInterviewRespond)({ sessionId, response, mode, pressureMode, transcript: transcript });
    ...
    if (cleanPath === '/api/interview/evaluate' && method === 'POST') {
        ...
        if (!transcript || transcript.length < 2)
            throw new ApiError(400, 'TOO_SHORT', 'Interview too short to evaluate. Please answer at least 2 questions.');
        const session = await fs.getInterviewSession(uid, sessionId);
        const evaluation = await (0, interviewAI_1.aiInterviewEvaluate)(transcript, session?.mode || 'hr');
        await fs.completeInterviewSession(uid, sessionId, evaluation);
        _transcripts.delete(sessionId);
        return { evaluation, sessionId };
    }
    if (cleanPath === '/api/interview/history' && method === 'GET') {
        const sessions = await fs.getInterviewHistory(uid);
        return { sessions };
    }
    if (cleanPath.startsWith('/api/interview'))
        return { ok: true };
    if (cleanPath.startsWith('/api/trust/score')) {
        const p = await fs.getUserProfile(uid);
        return { score: p?.trust_score || 75, breakdown: { missionAuthenticity: 80, examIntegrity: 90, behavioralConsistency: 75 }, signals: { documents: [], speakingMetrics: [] } };
    }
    if (cleanPath === '/api/trust/evaluate') {
        const p = await fs.getUserProfile(uid);
        const ns = Math.min(100, (p?.trust_score || 75) + 1);
        await fs.updateUserProfile(uid, { trust_score: ns });
        return { score: ns, ok: true };
    }
    if (cleanPath.startsWith('/api/trust'))
        return { ok: true, score: 75 };
    if (cleanPath === '/api/personality/report')
        return { traits: { confidence: 72, communication: 76, leadership: 58, discipline: 83, empathy: 68 }, insights: ['Strong discipline score.', 'Work on reducing filler words.'], weekly_change: { communication: 2 } };
    if (cleanPath === '/api/personality/session')
        return { challenge: "Introduce yourself as if walking into your dream company's first interview.", instructions: 'Cover: who you are, top skills, one achievement.' };
    if (cleanPath === '/api/personality/analyze')
        return { analysis: { summary: 'Good response structure.', trait_scores: { communication: 74, confidence: 68 }, strengths: ['Clear structure'], improvements: ['Reduce filler words'], filler_words: [], confidence_delta: 1 } };
    if (cleanPath.startsWith('/api/personality'))
        return { traits: {}, insights: [] };
    if (cleanPath === '/api/exam/available')
        return { exams: [{ id: 'exam-react-001', title: 'React Fundamentals Certification', exam_type: 'mcq', status: 'published', duration_minutes: 30, total_marks: 50, passing_marks: 35, question_count: 8, difficulty: 'Medium' }, { id: 'exam-python-001', title: 'Python Coding Challenge', exam_type: 'coding', status: 'published', duration_minutes: 45, total_marks: 60, passing_marks: 42, question_count: 3, difficulty: 'Medium' }] };
    if (cleanPath.includes('/api/exam/') && cleanPath.includes('/questions')) {
        const examId = cleanPath.split('/exam/')[1].replace('/questions', '');
        return { exam: examId === 'exam-react-001' ? { id: 'exam-react-001', title: 'React Fundamentals', durationMinutes: 30, totalMarks: 50, passingMarks: 35, allowedSwitches: 3, questions: [{ id: 'rq1', type: 'mcq', text: 'What hook manages state in React functional components?', marks: 5, options: ['useEffect', 'useState', 'useRef', 'useContext'] }, { id: 'rq2', type: 'mcq', text: 'Which is equivalent to componentDidMount?', marks: 5, options: ['useEffect w/deps', 'useEffect cleanup', 'useEffect with []', 'useReducer'] }, { id: 'rq3', type: 'essay', text: 'Explain the difference between controlled and uncontrolled components.', marks: 15 }] } : null };
    }
    if (cleanPath === '/api/exam/sync-result')
        return { ok: true, result: { percentage: 80, badge_level: 'gold' }, vaultItemId: `vault-${Date.now()}` };
    if (cleanPath === '/api/exam/results')
        return { results: [] };
    if (cleanPath.startsWith('/api/exam'))
        return { ok: true };
    ...
    if (cleanPath.startsWith('/api/sentinel'))
        return { ok: true };
    function buildTeacherSystemPrompt(teacherId, careerContext) {
...
- Keep your answers highly engaging, premium, and concise (under 120 words).`;
    ...
    async function callExternalLLM(messages, systemPrompt, skillCategory) {
    ...
    if (cleanPath === '/api/chat' && method === 'POST') {
        const { message, teacherId = 'priya' } = body;
        const teacherName = teacherId === 'priya' ? 'Ms. Priya' : teacherId === 'aisha' ? 'Ms. Aisha' : teacherId === 'rohan' ? 'Mr. Rohan' : 'Mr. Vikram';
        try {
    ...
    if (cleanPath === '/api/chat/session')
        return { sessionId: `chat-${Date.now()}`, opening: 'Hello! I\'m your AI study partner. What would you like to learn today?' };
    if (cleanPath.startsWith('/api/chat/history/'))
        return { messages: [] };
    if (cleanPath.startsWith('/api/chat'))
        return { ok: true };
    if (cleanPath === '/api/study/complete') {
        const p = await fs.getUserProfile(uid);
        const current = p?.consistency_score || 60;
        await fs.updateUserProfile(uid, { consistency_score: Math.min(100, current + 1) });
        return { ok: true };
    }
    if (cleanPath.startsWith('/api/study'))
        return { ok: true };
    ...
    if (cleanPath === '/api/recruiter/analytics') {
        ...
        const avgAts = total ? Math.round(students.reduce((acc, curr) => acc + (curr.ats_score || 50), 0) / total) : 50;
        const avgTrust = total ? Math.round(students.reduce((acc, curr) => acc + (curr.trust_score || 50), 0) / total) : 50;
        const avgDna = total ? Math.round(students.reduce((acc, curr) => acc + (curr.career_dna_score || 50), 0) / total) : 50;
        return {
            analytics: {
                avg_ats: avgAts,
                avg_trust: avgTrust,
                ...
                high_trust_count: students.filter(s => (s.trust_score || 50) >= 70).length,
                high_ats_count: students.filter(s => (s.ats_score || 50) >= 80).length
            }
        };
    }
    if (cleanPath.startsWith('/api/recruiter/candidates')) {
        const users = await fs.getAllUsers();
        const students = users.filter(u => u.role === 'student' && (u.recruiterVisible === true || (u.recruiter_visibility || 0) > 0));
        const formatted = students.map(s => ({
            id: s.id,
            display_name: s.displayName || s.username || 'Student',
            ats_score: s.ats_score || 50,
            trust_score: s.trust_score || 50,
            career_dna_score: s.career_dna_score || 50,
            mission_streak: s.mission_streak || 0,
            recruiter_visibility: s.recruiter_visibility || 65,
            communication_score: s.communication_score || 60,
            execution_score: s.execution_score || 60,
            skill_tags: s.skill_tags || ['React', 'Node.js'],
            missions_done: s.missions_completed || 0,
            interviews_done: s.interviews_done || 0
        }));
    ...
    if (cleanPath === '/api/recruiter/jobs') {
        ...
        if (method === 'POST') {
            const id = await fs.addJob(uid, body);
            return { ok: true, id };
        }
    }
    ...
    if (cleanPath.startsWith('/api/recruiter/candidate/')) {
        ...
        const profile = await fs.getUserProfile(candidateId);
    ...
    if (cleanPath.startsWith('/api/recruiter/visibility')) {
        const { visible } = body;
        await fs.updateUserProfile(uid, { recruiter_visibility: visible ? 80 : 0 });
        return { ok: true };
    }
    if (cleanPath.startsWith('/api/recruiter'))
        return { ok: true, candidates: [], requests: [], interviews: [] };
    if (cleanPath === '/api/university/dashboard') {
        return { placementStats: { total_students: 240, placed: 168, avg_ctc: 8.4, top_recruiters: ['TCS', 'Infosys', 'Wipro'] }, topStudents: [], deptStats: [] };
    }
    if (cleanPath === '/api/university/employability-report') {
        return { report: { employability_score: 72, industry_readiness: 68 } };
    }
    if (cleanPath === '/api/university/skill-gaps') {
        return { gaps: [{ skill: 'System Design', demand: 88, supply: 42 }, { skill: 'DSA', demand: 92, supply: 55 }] };
    }
    if (cleanPath.startsWith('/api/university'))
        return { ok: true, placementStats: { total_students: 0 }, topStudents: [], deptStats: [], report: {}, gaps: [] };
    ...
    if (cleanPath === '/api/consultant/pipeline') {
        ...
        const pipeline = { onboarding: [], document_collection: [], application: [], visa: [], pre_departure: [], completed: [] };
        for (const s of students) {
            const status = s.status || 'onboarding';
            if (pipeline[status]) {
                ...
                pipeline[status].push({
                    ...
                    targetCountry: s.targetCountry || 'USA',
                    programType: s.programType || 'Masters',
        ...
        return { pipeline, stats: { total: students.length, active: students.filter(s => s.status !== 'completed').length } };
    }
    if (cleanPath === '/api/consultant/student/add') {
        ...
        try {
            ...
            const { data: signUpData, error: signUpErr } = await supabaseClient_1.supabase.auth.signUp({
                email,
            ...
            if (signUpData?.user) {
                ...
                if (signUpErr.message?.includes('already registered') || signUpErr.message?.includes('already exists') || signUpErr.status === 422) {
                    const { data: signInData, error: signInErr } = await supabaseClient_1.supabase.auth.signInWithPassword({
                        email,
                        password: 'TemporaryPassword123!'
                    });
                    if (signInData?.user) {
                        ...
                        const { data: retryData, error: retryErr } = await supabaseClient_1.supabase.auth.signUp({
                            email: uniqueEmail,
                            password: 'TemporaryPassword123!',
                            options: {
                                data: {
    ...
    if (cleanPath.startsWith('/api/consultant/student/') && cleanPath.endsWith('/task') && method === 'POST') {
        ...
        const task = body;
        const profile = await fs.getUserProfile(studentId);
    ...
    if (cleanPath === '/api/consultant/sessions') {
        ...
        if (method === 'POST') {
            const id = await fs.scheduleSession({ ...body, consultantId: uid });
            return { ok: true, id };
        }
    }
    ...
    if (cleanPath === '/api/admin/users') {
        ...
        if (searchVal) {
            filteredUsers = filteredUsers.filter(u => u.displayName?.toLowerCase().includes(searchVal) ||
        ...
        const formatted = filteredUsers.map(u => ({
            ...
            ats_score: u.ats_score || 50,
            trust_score: u.trust_score || 50,
            career_dna_score: u.career_dna_score || 50,
    ...
    if (cleanPath.startsWith('/api/admin'))
        return { ok: true, users: [], log: [], total: 0 };
    if (cleanPath.startsWith('/api/memory'))
        return { ok: true };
    if (cleanPath.startsWith('/api/tts'))
        return { ok: true };
    if (cleanPath === '/api/avatar/chat' && method === 'POST') {
        const { message, history = [], teacherId = 'priya', careerContext } = body;
        const teacherName = teacherId === 'priya' ? 'Ms. Priya' : teacherId === 'aisha' ? 'Ms. Aisha' : teacherId === 'rohan' ? 'Mr. Rohan' : 'Mr. Vikram';
        try {
    ...
    if (cleanPath.startsWith('/api/avatar'))
        return { ok: true };
    if (cleanPath.startsWith('/api/notes'))
        return { notes: [] };
...
async function request(method, path, body) {
    try {
        return await firestoreRouter(method, path, body);
    }
    catch (err) {
        if (err instanceof ApiError)
            throw err;
        throw new ApiError(500, 'FIRESTORE_ERROR', err.message || 'Request failed');
    }
}
exports.api = {
    get: (path, _opts) => request('GET', path),
